...
<br><center><span class='judulcontent'>BACKUP DATA UO TNI AD </span></center><br>
...
//	$file='Backup'.'_'.date("dmY").'_'.date('His').'_'.$nama1.'.sql';
	$file='Backup'.'_'.date("dmY").'_'.date('His').'_'.'UO'.'.sql';
//	$file=date("dmY").'_'.'watzah'.'_'.time().'.sql';
    ...
    <p align="center"><a style="cursor:pointer" onclick="location.href='backup/download_backup_data1.php?nama_file=<?php echo $file;?>'" title="Download"><center><span class="judulsubcontent">Hasil Backup Tersimpan di c:/xampp/htdocs/laplakgar2022/adk</span></center></a>
	<?php
}else{
	unset($_POST['backup']);
}
...
function backup_tables($host,$user,$pass,$name,$nama_file,$tables = 'dipa,bpjs, yanmasum,blu, hibah, kapitasi, realisasi, realisasix, realisasi_bpjs, realisasi_yanmasum, realisasi_blu, realisasi_kapitasi,  realisasi_hibah, tunkin, pengembalian, kopstuk, tajuk_ttd, lamp_laplakgar,  surat, tembusan, pnbpum, realisasi_pnbpum, laphibah, sbsn, realisasi_sbsn')

...
print "<br><table  width='1100' align='center' ><tr><td class='judulsubcontent' align='center'>REALISASI ANGGARAN DIPA PETIKAN SATKER<br> 
BULAN $nmbulan TAHUN $_GET[thang]</td></tr></table><br>";	

$query= "SELECT '' as pengembalian, '' as id_pagu,  a.kdwasgiat, a.kdkotama, a.kdsatker, a.thang, '' as kode, '1' as display,  'RUPIAH MURNI' as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi,  '' as id_realisasi, c.blnlalu, d.blnini
FROM dipa a 
left join(select kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi where kdbulan<'$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by kdkotama,kdsatker,thang) as c on a.kdkotama=c.kdkotama and a.kdsatker=c.kdsatker and a.thang=c.thang 
left join(select kdkotama, kdsatker, thang, sum(realisasi)  as blnini from realisasi where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by kdkotama,kdsatker,thang) as d on a.kdkotama=d.kdkotama and a.kdsatker=d.kdsatker and a.thang=d.thang
...
left join (select kdprogram, kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi where kdbulan<'$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by kdprogram) as c on a.kdprogram=c.kdprogram
left join (select kdprogram, kdkotama, kdsatker, thang, sum(realisasi) as  blnini from realisasi where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by kdprogram) as d on a.kdprogram=d.kdprogram
where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]' 
...
FROM dipa  a
left join (select  id_pagu, id_realisasi, sum(realisasi) as blnlalu from realisasi where kdbulan<'$_GET[kdbulan]'  and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by id_pagu) as c on a.id_pagu=c.id_pagu
left join (select id_pagu, sum(realisasi) as blnini  from realisasi where kdbulan='$_GET[kdbulan]'  and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by id_pagu) as d on a.id_pagu=d.id_pagu
...
print "<form name='form1' method='GET'  action='realisasi/kirimparameter.php' >";
	...
	print "<form name='form1' method='GET'  action='cetak/cetak_prog_daerah_satker.php' target='_Blank'>"; 
...
while($k = mysql_fetch_array($ok)){
	...
	if ($pj=='3') {	
	 ...
	 print "<td  valign='top' align='right'></td>";
		  ...
		  print "<td  valign='top' >"; 
				 if ($pj<'20') print "<b>$k[uraian]</b>"; else 
		...
		print	"<td  valign='top' align='center'>"; if ($pj>'20') print "<a href='media.php?module=inputrealisasi&id_pagu=$k[id_pagu]&kdbulan=$_GET[kdbulan]&thang=$_GET[thang]' data-tooltip='Realisasi' data-position='top' class='top'>
		<img src='images/r.png' width='20' ></a>"; else print ""; print "</td>"; 	

		print	"<td  valign='top' align='center'>"; 
		if (($pj>'20') and ($k[pengembalian]==''))  
			print "<a href='realisasi/proses.php?aksi=ubahstatus&id_pagu=$k[id_pagu]&id_realisasi=$k[id_realisasi]&kdbulan=$_GET[kdbulan]&thang=$_GET[thang]' data-tooltip='Pengembalian' data-position='top' class='top'>
		    <img src='images/rp.png' width='20' ></a>"; 
	    else if (($pj>'20') and ($k[pengembalian]=='x')) 
			print "<a href='realisasi/proses.php?aksi=ubahstatuskosong&id_pagu=$k[id_pagu]&id_realisasi=$k[id_realisasi]&kdbulan=$_GET[kdbulan]&thang=$_GET[thang]' data-tooltip='Pengembalian' data-position='top' class='top'><img src='images/chek1.png' width='20' ></a>"; 	

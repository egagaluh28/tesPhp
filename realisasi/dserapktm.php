...
<?php
print "<form name='form1' method='GET'  action='realisasi/kirimparameter_dayaserapktm.php' >";
...
//	print "&nbsp;<a href='c3t4k/cetakdserapuo.php?kdbulan=$_GET[kdbulan]&tahun=$_GET[thang]'><input  type='button' value='&nbsp;Cetak&nbsp;' /></a>";
	...
	print "<form name='form1' method='GET'  action='cetak/cetakdserapktm.php' target='_Blank'>"; 
...
left join (select kdkotama, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi from dipa where kdkotama='$_SESSION[kdkotama]' and thang='$_GET[thang]' group by kdkotama, thang) as b on a.kdkotama=b.kdkotama
left join (select kdkotama, sum(realisasi) as blnlalu from realisasi where kdkotama='$_SESSION[kdkotama]' and kdbulan<'$_GET[kdbulan]' and thang='$_GET[thang]' group by kdkotama, thang) as c on a.kdkotama=c.kdkotama
left join (select kdkotama, sum(realisasi) as blnini from realisasi where kdkotama='$_SESSION[kdkotama]' and kdbulan='$_GET[kdbulan]' and thang='$_GET[thang]' group by kdkotama, thang) as d on a.kdkotama=d.kdkotama where a.kdkotama='$_SESSION[kdkotama]'
union
...
left join (select kdsatker, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi from dipa where kdkotama='$_SESSION[kdkotama]' and  thang='$_GET[thang]' group by kdsatker, thang) as b on a.kdsatkr=b.kdsatker
left join (select kdsatker, sum(realisasi) as blnlalu from realisasi where kdkotama='$_SESSION[kdkotama]' and kdbulan<'$_GET[kdbulan]' and thang='$_GET[thang]' group by kdsatker, thang) as c on a.kdsatkr=c.kdsatker
left join (select kdsatker, sum(realisasi) as blnini from realisasi where kdkotama='$_SESSION[kdkotama]' and  kdbulan='$_GET[kdbulan]' and thang='$_GET[thang]' group by kdsatker, thang) as d on a.kdsatkr=d.kdsatker

...
define('FPDF_FONTPATH','../cetak/fpdf/font/');
require('../cetak/fpdf/fpdf.php');
...
select a.kdsatker as kdakun, a.kdsatker as display, b.nmsatkr as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, '' as nospm, '' as tglspm, sum(a1.realisasi) as realisasi from bpjs a
...
union (select a.kdakun, concat(a.kdsatker, a.kdsakun,a.urutitem,a.id_pagu) as display, concat('- ',a.nmitem) as uraian, a.pagu, a.revisi, a.pagurevisi, '' as nospm, '' as tglspm, x.realisasi from bpjs a
	  left join (select id_pagu, kdkotama, kdsatker, thang,  sum(realisasi) as realisasi from   realisasi_bpjs where kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' group by id_pagu) as x on  a.id_pagu=x.id_pagu
where a.kdsa		='$_GET[kdsa]' and
	  a.kdprogram	='$_GET[kdprogram]' and
	  a.kdgiat   	='$_GET[kdgiat]' and
	  a.kdoutput   	='$_GET[kdoutput]' and
	  ...
	  a.thang    	='$_GET[thang]' 
	  group by a.id_pagu) 	 
union(select a.kdakun, concat(a.kdsatker, a.kdsakun,a.urutitem,a.id_pagu,y.id_realisasi) as display, '' as uraian, ''as pagu, '' as revisi, '' as pagurevisi, y.nospm, y.tglspm, y.realisasi from bpjs a
	  left join (select id_pagu, id_realisasi, kdkotama, kdsatker, thang, nospm, tglspm, realisasi from   realisasi_bpjs where kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' ) as y on  a.id_pagu=y.id_pagu
where a.kdsa		='$_GET[kdsa]' and
	  a.kdprogram	='$_GET[kdprogram]' and
	  a.kdgiat   	='$_GET[kdgiat]' and
	  a.kdoutput   	='$_GET[kdoutput]' and
...
left join (select id_pagu, kdkotama, kdsatker, thang, sum(realisasi) as realisasi from  realisasi_bpjs where kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' group by id_pagu) as b on  a.id_pagu=b.id_pagu
where a.kdsa	='$_GET[kdsa]' and
	  a.kdprogram	='$_GET[kdprogram]' and
	  a.kdgiat   	='$_GET[kdgiat]' and
	  a.kdoutput   	='$_GET[kdoutput]' and
...
$hasil = mysql_fetch_array($jml);	
   ...
   $sql=mysql_query("SELECT * FROM tajuk_ttd where kdkotama='$_GET[kdkotama]' and kdsatker='000000' order by thang desc, kdbulan desc limit 1"); 
$row = mysql_fetch_array($sql);

...
$pdf->Cell($grs,0.2,'                             ',0,0,'C',1); 
	...
	// $row = mysql_fetch_array($sql);
	 ...
	 $ambilkode = mysql_query("SELECT a.thang, a.kdkotama, a.kdsatker, a.kdsa, a.kdprogram, a.kdgiat, a.kdoutput, a.kdakun, a.nmakun, b.nmprogram, c.nmgiat, d.nmoutput, f.nmsa from blu a
...
$pdf->SetY(25);
$pdf->SetX(110); 
$pdf->Cell(60,5,'KARTU PENGAWASAN ANGGARAN TNI AD'.' '.$thang.' (BLU) ',0,1,'L');
$pdf->SetY(30); 
$pdf->SetX(110); 
...
from blu a 
left join (select id_pagu, kdkotama, kdsatker, thang, sum(realisasi) as realisasi from  realisasi_blu where 
...
union (select a.kdakun, concat(a.kdsakun,a.urutitem,a.id_pagu) as display, concat('- ',a.nmitem) as uraian, a.pagu, a.revisi, a.pagurevisi, '' as nospm, '' as tglspm, x.realisasi from blu a
	  left join (select id_pagu, kdkotama, kdsatker, thang,  sum(realisasi) as realisasi from   realisasi_blu where kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by id_pagu) as x on  a.id_pagu=x.id_pagu
where a.kdsa		='$_GET[kdsa]' and
	  a.kdprogram	='$_GET[kdprogram]' and
	  a.kdgiat   	='$_GET[kdgiat]' and
	  a.kdoutput   	='$_GET[kdoutput]' and
	  ...
	  a.thang    	='$_GET[thang]' 
	  group by a.id_pagu) 	 
union(select a.kdakun, concat(a.kdsakun,a.urutitem,a.id_pagu,y.id_realisasi) as display, '' as uraian, ''as pagu, '' as revisi, '' as pagurevisi, y.nospm, y.tglspm, y.realisasi from blu a
	  left join (select id_pagu, id_realisasi, kdkotama, kdsatker, thang, nospm, tglspm, realisasi from   realisasi_blu where kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' ) as y on  a.id_pagu=y.id_pagu
where a.kdsa		='$_GET[kdsa]' and
	  a.kdprogram	='$_GET[kdprogram]' and
	  a.kdgiat   	='$_GET[kdgiat]' and
	  a.kdoutput   	='$_GET[kdoutput]' and
...
$jml= mysql_query("SELECT  sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, sum(b.realisasi) as realisasi from blu a 
left join (select id_pagu, kdkotama, kdsatker, thang, sum(realisasi) as realisasi from  realisasi_blu where kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by id_pagu) as b on  a.id_pagu=b.id_pagu
where a.kdsa	='$_GET[kdsa]' and
	  a.kdprogram	='$_GET[kdprogram]' and
	  a.kdgiat   	='$_GET[kdgiat]' and
	  a.kdoutput   	='$_GET[kdoutput]' and

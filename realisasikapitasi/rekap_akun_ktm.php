...
<html>
	...
	<br><center><span class='judulcontent'>REKAP PER AKUN (KAPITASI)</span></center><br>
...
//        pakai ini
            ...
            $reload = "media.php?module=rekap_akun_kapitasi_ktm&thang=$_GET[thang]&kdbulan=$_GET[kdbulan]&pagination=true&keyword=$keyword";
            $result =  mysql_query("select z.* from(SELECT  a.kdkotama,  a.thang, a.kdakun, a.nmakun,  
sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi,    c.blnlalu, d.blnini
FROM kapitasi a 
left join (select kdgiat, kdoutput, kdakun,  kdkotama, thang, sum(realisasi) as blnlalu from realisasi_kapitasi where kdbulan<'$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and thang='$_GET[thang]' group by  kdakun) as c on   a.kdakun=c.kdakun 
left join (select kdgiat, kdoutput, kdakun,  kdkotama, thang, sum(realisasi) as  blnini from realisasi_kapitasi where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and thang='$_GET[thang]' group by  kdakun) as d on   a.kdakun=d.kdakun 
...
//            jika tidak ada pencarian pakai ini
            $reload = "media.php?module=rekap_akun_kapitasi_ktm&thang=$_GET[thang]&kdbulan=$_GET[kdbulan]&pagination=true";
            $result =  mysql_query("SELECT  a.kdkotama,  a.thang, a.kdakun, a.nmakun,  
sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi,    c.blnlalu, d.blnini
FROM kapitasi a 
left join (select kdgiat, kdoutput, kdakun,  kdkotama, thang, sum(realisasi) as blnlalu from realisasi_kapitasi where kdbulan<'$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and thang='$_GET[thang]' group by  kdakun) as c on   a.kdakun=c.kdakun 
left join (select kdgiat, kdoutput, kdakun,  kdkotama, thang, sum(realisasi) as  blnini from realisasi_kapitasi where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and thang='$_GET[thang]' group by  kdakun) as d on   a.kdakun=d.kdakun 
...
<table  width='98%' align='center'>
    <tr>
		 ...
		 <form name='form1' method='GET'  action='realisasikapitasi/kirimparameter_rekapakun_ktm.php' >
	...
	print "&nbsp;&nbsp;<a href='cetak/cetak_rekapakun_kapitasi_ktm.php?kdbulan=$_GET[kdbulan]&thang=$_GET[thang]&kdkotama=$_SESSION[kdkotama]&kdsatker=$_SESSION[kdsatker]' target='_blank' style='text-decoration:none'><input  type='button' value='Cetak' class='button2'/></a>";
	?> 
		...
		<td align="right">
			...
			<a href="<?php print "media.php?module=rekap_akun_kapitasi_ktm&thang=$_GET[thang]&kdbulan=$_GET[kdbulan]"; ?>"

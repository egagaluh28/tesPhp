...
$(document).ready(function(){
  //apabila terjadi event onchange terhadap object <select id=kdkotama>
	 ...
	 $("#kdkotama1").change(function(){
		var kdkotama1 = $("#kdkotama1").val();
		$.ajax({
			url: "form_uo/ambilsatker1.php",
			data: "kdkotama1="+kdkotama1,
			cache: false,
			success: function(msg){
				$("#kdsatkr1").html(msg);
			}
 ...
 <center><br><span class='judulcontent'>CETAK DIPA PETIKAN SATKER</span></center><br><br>
 ...
 print "<form name='form1' method='GET'  action='cetak/cetak_daerah_satker_jenbel.php' target='_blank'>";
	...
	<center><br><span class='judulcontent'>CETAK DIPA PETIKAN SATKER (PLUS SBSN)</span></center><br><br>
 ...
 print "<form name='form1' method='GET'  action='cetak/cetak_daerah_satker_jenbel_sbsn.php' target='_blank'>";
    print "<table width='80%' align='center' cellpadding='3'  >";
	 print  	"<tr>
				 ...
				 <td><select name='kdkotama' id='kdkotama1' class='select-field' required='required' >
						<option value='' selected> - - Pilih Kotama - - </option>";
						 $sql="select kdkotama, nmkotama from t_kotam order by kdkotama";
						 $qry=mysql_query($sql);
						 while ($data=mysql_fetch_array($qry)){
	...
	print "<td class='subyek1' align='right' width='40%'>";
	print "SATKER :</td> 
	      <td><select name='kdsatker'  id='kdsatkr1'  class='select-field' required='required'>";
						print "<option value='' selected>- - Pilih Satker - -</option>";

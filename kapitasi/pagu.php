...
print "<br><table  width='1100' align='center' ><tr><td class='judulcontent' align='center'>PEREKAMAN DATA PAGU - KAPITASI $_GET[thang]</td></tr></table>";	

$query= "SELECT '' as id_pagu,  '' as kdprogram, kdwasgiat, kdkotama, kdsatker, thang, '' as kode, '1' as display,  'KAPITASI' as uraian, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi
FROM kapitasi where  kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' 
...
union (SELECT id_pagu, '' as kdprogram, kdwasgiat, kdkotama, kdsatker, thang, '' as kode, concat('1',kdprogram,kdgiat,kdoutput,kdakun, kdsakun,urutitem) as display, concat('-',' ', nmitem) as uraian, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  FROM kapitasi  where  kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by id_pagu order by noitem, id_pagu) order by display";


		$ok = mysql_query($query);
 
print "<table width='80%' align='center'><tr><td><a href='media.php?module=inputpagukapitasi&kdkotama=$_SESSION[kdkotama]&kdsatker=$_SESSION[kdsatker]&thang=$_GET[thang]' style='text-decoration:none'><div class='codehim-tombol-biru' ><input  type='button' value='Tambah Kegiatan'/></div></a></td></tr></table><br>";
...
while($k = mysql_fetch_array($ok)){
	...
	if ($pj=='3') {	
	 ...
	 print "<td  valign='top' align='right'></td>";
		...
		if ($pj=='19') { print "<td colspan='2' align='center'><a href='media.php?module=rekamdetailkapitasi&kdprogram=$k[kdprogram]&kdgiat=$kd2&kdoutput=$kd3&kdakun=$kd4&kdsakun=$kd5&kdkotama=$_SESSION[kdkotama]&kdsatker=$_SESSION[kdsatker]&thang=$_GET[thang]' data-tooltip='Tambah Uraian' data-position='top' class='top'>
		<img src='images/add.png' width='20' ></a></td>"; 
		
		} else { 
	
		print	"<td  valign='top' align='center'>"; if ($pj>'20') print "<a href='media.php?module=editpagukapitasi&id_pagu=$k[id_pagu]&thang=$k[thang]' data-tooltip='Edit Pagu' data-position='top' class='top'>
		<img src='images/edit.png' width='20' ></a>"; else print ""; print "</td>"; 
		print"<td  valign='top' align='center'>"; if ($pj>'20') print "<a href=\"kapitasi/proses.php?aksi=hapus&id_pagu=$k[id_pagu]&thang=$k[thang]\" 

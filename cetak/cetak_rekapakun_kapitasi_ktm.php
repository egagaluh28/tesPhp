...
$pdf->Cell(12,5,'Tanggal',0,0,'L',1);
	 ...
	 $sql= mysql_query("SELECT  a.kdkotama,  a.thang, a.kdakun, a.nmakun,  
sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi,    c.blnlalu, d.blnini
FROM kapitasi a 
left join (select kdgiat, kdoutput, kdakun,  kdkotama, thang, sum(realisasi) as blnlalu from realisasi_kapitasi where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' group by  kdakun) as c on   a.kdakun=c.kdakun 
left join (select kdgiat, kdoutput, kdakun,  kdkotama, thang, sum(realisasi) as  blnini from realisasi_kapitasi where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' group by  kdakun) as d on   a.kdakun=d.kdakun 

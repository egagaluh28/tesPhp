...
FROM hibah a 
left join(select a.kdkotama, a.thang, sum(b.realisasi) as blnlalu from hibah a left join realisasi_hibah b on a.id_pagu=b.id_pagu where b.kdbulan<'$_GET[kdbulan]' and a.thang='$_GET[thang]' group by a.thang) as c on a.thang=c.thang 
left join(select a.kdkotama, a.thang, sum(b.realisasi) as blnini  from hibah a left join realisasi_hibah b on a.id_pagu=b.id_pagu where b.kdbulan='$_GET[kdbulan]'  and a.thang='$_GET[thang]' group by a.thang) as d on a.thang=d.thang 
...
left join (select a.kdsatker, a.kdkotama,  a.thang, sum(b.realisasi) as blnlalu from hibah a left join realisasi_hibah b on a.id_pagu=b.id_pagu where b.kdbulan<'$_GET[kdbulan]' and a.thang='$_GET[thang]' group by a.kdkotama) as c on  a.kdkotama=c.kdkotama and a.thang=c.thang 
left join (select a.kdsatker, a.kdkotama,  a.thang, sum(b.realisasi) as blnini from hibah a left join realisasi_hibah b on a.id_pagu=b.id_pagu where b.kdbulan='$_GET[kdbulan]' and a.thang='$_GET[thang]' group by a.kdkotama) as d on  a.kdkotama=d.kdkotama and a.thang=d.thang 
where   a.thang='$_GET[thang]' 
group by a.kdkotama)
...
left join (select a.kdsatker, a.kdkotama,  a.thang, sum(b.realisasi) as blnlalu from hibah a left join realisasi_hibah b on a.id_pagu=b.id_pagu where b.kdbulan<'$_GET[kdbulan]' and a.thang='$_GET[thang]' group by a.kdkotama,a.kdsatker) as c on  a.kdkotama=c.kdkotama and a.thang=c.thang and  a.kdsatker=c.kdsatker
left join (select a.kdsatker, a.kdkotama,  a.thang, sum(b.realisasi) as blnini from hibah a left join realisasi_hibah b on a.id_pagu=b.id_pagu where b.kdbulan='$_GET[kdbulan]' and a.thang='$_GET[thang]' group by a.kdkotama,a.kdsatker) as d on  a.kdkotama=d.kdkotama and a.thang=d.thang and  a.kdsatker=d.kdsatker
where  a.thang='$_GET[thang]' 
...
left join (select a.kdgiat, a.kdoutput, a.kdakun,  a.kdkotama,  a.kdsatker, a.thang, sum(b.realisasi) as blnini from hibah a left join realisasi_hibah b on a.id_pagu=b.id_pagu where b.kdbulan='$_GET[kdbulan]'   
...
$pdf->SetY(35);
$pdf->SetFont('Arial','',9);
$pdf->Cell(0,5,'PELAKSANAAN ANGGARAN HIBAH',0,1,'C');
$pdf->SetY(40); 
...
from hibah a 
left join (select id_pagu, thang, sum(realisasi) as blnlalu from realisasi_hibah  where kdbulan<'$_GET[kdbulan]' and  
thang='$_GET[thang]' group by thang) as c on  a.thang=c.thang 
left join (select id_pagu,   thang, sum(realisasi) as blnini from realisasi_hibah where kdbulan='$_GET[kdbulan]' and  
...
left join (select kdjenbel,   thang, sum(realisasi) as blnlalu  from realisasi_hibah where kdbulan<'$_GET[kdbulan]' and   thang='$_GET[thang]' group by  kdjenbel) as c on a.kdjenbel=c.kdjenbel 
left join (select  kdjenbel,  thang, sum(realisasi) as blnini from realisasi_hibah where kdbulan='$_GET[kdbulan]' and   thang='$_GET[thang]' group by kdjenbel) as d on a.kdjenbel=d.kdjenbel 

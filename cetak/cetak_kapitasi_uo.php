...
$pdf->Cell(12,5,'Tanggal',0,0,'L',1);
	 ...
	 $sql= mysql_query("SELECT '' as id_pagu,  a.kdwasgiat,  a.thang, '' as kode, '1' as display,  'KAPITASI' as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.blnlalu, d.blnini
FROM kapitasi a 
left join(select  thang, sum(realisasi) as blnlalu from  realisasi_kapitasi  where kdbulan<'$_GET[kdbulan]'  and thang='$_GET[thang]' group by thang) as c on  a.thang=c.thang 
left join(select  thang, sum(realisasi) as blnini from   realisasi_kapitasi  where kdbulan='$_GET[kdbulan]'  and thang='$_GET[thang]' group by thang) as d on  a.thang=d.thang 
...
left join (select kdprogram,    thang, sum(realisasi) as blnlalu from realisasi_kapitasi where kdbulan<'$_GET[kdbulan]' and  thang='$_GET[thang]' group by kdprogram) as c on  a.kdprogram=c.kdprogram
left join (select kdprogram,    thang, sum(realisasi) as blnini from realisasi_kapitasi  where kdbulan='$_GET[kdbulan]' and  thang='$_GET[thang]' group by kdprogram) as d on  a.kdprogram=d.kdprogram
...
left join (select kdprogram, kdgiat, thang, sum(realisasi) as blnlalu from realisasi_kapitasi  where kdbulan<'$_GET[kdbulan]' and  thang='$_GET[thang]' group by kdprogram,kdgiat) as c on  a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat
left join (select kdprogram, kdgiat,    thang, sum(realisasi) as blnini from  realisasi_kapitasi where kdbulan='$_GET[kdbulan]'  and thang='$_GET[thang]' group by kdprogram,kdgiat) as d on  a.kdprogram=d.kdprogram and  a.kdgiat=d.kdgiat
where  a.thang='$_GET[thang]' 
group by a.kdprogram,a.kdgiat)
union (SELECT '' as id_pagu, a.kdwasgiat,    a.thang, a.kdoutput as kode, concat('1',a.kdprogram,a.kdgiat,a.kdoutput) as display, b.nmoutput as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.blnlalu, d.blnini FROM kapitasi a 
left join t_output b on a.kdprogram=b.kdprogram and a.kdgiat=b.kdgiat and a.kdoutput=b.kdoutput
left join (select kdprogram, kdgiat, kdoutput,   thang, sum(realisasi) as blnlalu from realisasi_kapitasi  where kdbulan<'$_GET[kdbulan]'   and thang='$_GET[thang]' group by kdprogram,kdgiat,kdoutput) as c on  a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat  and a.kdoutput=c.kdoutput
left join (select kdprogram, kdgiat,  kdoutput,    thang, sum(realisasi) as blnini from  realisasi_kapitasi where kdbulan='$_GET[kdbulan]'  and thang='$_GET[thang]' group by kdprogram,kdgiat,kdoutput) as d on  a.kdprogram=d.kdprogram and  a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput
...
FROM kapitasi a 
left join (select kdgiat, kdoutput, kdakun,    thang, sum(realisasi) as blnlalu from realisasi_kapitasi where kdbulan<'$_GET[kdbulan]'  and thang='$_GET[thang]' group by kdgiat,kdoutput, kdakun) as c on  a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput and a.kdakun=c.kdakun 
left join (select kdgiat, kdoutput, kdakun,   thang, sum(realisasi) as blnini from realisasi_kapitasi where kdbulan='$_GET[kdbulan]' and thang='$_GET[thang]' group by kdgiat,kdoutput, kdakun) as d on  a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput and a.kdakun=d.kdakun 
...
sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.blnlalu, d.blnini
FROM kapitasi a 
left join (select kdgiat, kdoutput, kdakun, kdsakun,    thang, sum(realisasi) as blnlalu from realisasi_kapitasi where kdbulan<'$_GET[kdbulan]'  and  thang='$_GET[thang]' group by kdgiat,kdoutput, kdakun,kdsakun) as c on  a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput and a.kdakun=c.kdakun and a.kdsakun=c.kdsakun 
left join (select kdgiat, kdoutput, kdakun, kdsakun,  thang, sum(realisasi) as blnini from realisasi_kapitasi  where kdbulan='$_GET[kdbulan]'   and  thang='$_GET[thang]' group by kdgiat,kdoutput, kdakun,kdsakun) as d on  a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput and a.kdakun=d.kdakun and a.kdsakun=d.kdsakun 
...
$pdf->SetY(35);
$pdf->SetFont('Arial','',9);
$pdf->Cell(0,5,'PELAKSANAAN ANGGARAN DANA KAPITASI',0,1,'C');
$pdf->SetY(40); 
...
from kapitasi a 
left join (select id_pagu, thang, sum(realisasi) as blnlalu from realisasi_kapitasi  where kdbulan<'$_GET[kdbulan]' and  
thang='$_GET[thang]' group by thang) as c on  a.thang=c.thang 
left join (select id_pagu,   thang, sum(realisasi) as blnini from realisasi_kapitasi where kdbulan='$_GET[kdbulan]' and  
...
left join (select kdjenbel,   thang, sum(realisasi) as blnlalu  from realisasi_kapitasi where kdbulan<'$_GET[kdbulan]' and   thang='$_GET[thang]' group by  kdjenbel) as c on a.kdjenbel=c.kdjenbel 
left join (select  kdjenbel,  thang, sum(realisasi) as blnini from realisasi_kapitasi where kdbulan='$_GET[kdbulan]' and   thang='$_GET[thang]' group by kdjenbel) as d on a.kdjenbel=d.kdjenbel 

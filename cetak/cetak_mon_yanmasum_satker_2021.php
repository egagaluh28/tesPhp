...
$pdf->Cell(12,5,'Tanggal',0,0,'L',1);
	 ...
	 $sql= mysql_query("SELECT  '' as id_pagu,  a.kdwasgiat, a.kdkotama, a.kdsatker, a.thang, '' as kode, '1' as display,  'YANMASUM' as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi,  '' as id_realisasi, c.blnlalu, d.blnini
FROM yanmasum a 
left join(select kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi_yanmasum where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdkotama,kdsatker,thang) as c on a.kdkotama=c.kdkotama and a.kdsatker=c.kdsatker and a.thang=c.thang 
left join(select kdkotama, kdsatker, thang, sum(realisasi)  as blnini from realisasi_yanmasum where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdkotama,kdsatker,thang) as d on a.kdkotama=d.kdkotama and a.kdsatker=d.kdsatker and a.thang=d.thang
...
left join (select kdprogram, kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi_yanmasum where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdprogram) as c on a.kdprogram=c.kdprogram
left join (select kdprogram, kdkotama, kdsatker, thang, sum(realisasi) as  blnini from realisasi_yanmasum where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdprogram) as d on a.kdprogram=d.kdprogram
where  a.kdkotama='$_GET[kdkotama]' and a.kdsatker='$_GET[kdsatker]' and a.thang='$_GET[thang]' 
...
left join (select kdprogram, kdgiat, kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi_yanmasum 
...
left join (select kdprogram, kdgiat, kdkotama, kdsatker, thang, sum(realisasi)  as blnini from realisasi_yanmasum 
...
left join (select kdprogram, kdgiat, kdoutput, kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi_yanmasum 
...
left join (select kdprogram, kdgiat, kdoutput, kdkotama, kdsatker, thang, sum(realisasi)  as blnini from realisasi_yanmasum 
...
FROM yanmasum a 
left join (select kdgiat, kdoutput, kdakun,  kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi_yanmasum where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdgiat,kdoutput, kdakun) as c on  a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput and a.kdakun=c.kdakun 
left join (select kdgiat, kdoutput, kdakun,  kdkotama, kdsatker, thang, sum(realisasi) as  blnini from realisasi_yanmasum where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdgiat,kdoutput, kdakun) as d on  a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput and a.kdakun=d.kdakun 
where  a.kdkotama='$_GET[kdkotama]' and a.kdsatker='$_GET[kdsatker]' and a.thang='$_GET[thang]' 
...
FROM yanmasum  a
left join (select  id_pagu, id_realisasi, sum(realisasi) as blnlalu from realisasi_yanmasum where kdbulan<'$_GET[kdbulan]'  and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by id_pagu) as c on a.id_pagu=c.id_pagu
left join (select id_pagu, sum(realisasi) as blnini  from realisasi_yanmasum where kdbulan='$_GET[kdbulan]'  and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by id_pagu) as d on a.id_pagu=d.id_pagu
...
$pdf->SetY(35);
$pdf->SetFont('Arial','',9);
$pdf->Cell(0,5,'PELAKSANAAN ANGGARAN  DANA PNBP YANMASUM KESEHATAN',0,1,'C');
$pdf->SetY(40); 
...
left join (select id_pagu,kdkotama, kdsatker,  thang, sum(realisasi) as blnlalu from realisasi_yanmasum where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdkotama, kdsatker, thang) as c on a.kdkotama=c.kdkotama and a.kdsatker=c.kdsatker and a.thang=c.thang 

left join (select id_pagu,kdkotama, kdsatker,  thang, sum(realisasi) as blnini from realisasi_yanmasum where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdkotama, kdsatker, thang) as d on a.kdkotama=d.kdkotama and a.kdsatker=d.kdsatker and a.thang=d.thang 
...
left join (select a.kdkotama, a.kdsatker, a.thang, sum(a.realisasi) as blnlalu, b.kdjenbel from realisasi a left join yanmasum b on a.id_pagu=b.id_pagu where a.kdbulan<'$_GET[kdbulan]' group by a.kdkotama, a.kdsatker, a.thang, b.kdjenbel) as c on a.kdkotama=c.kdkotama and a.kdsatker=c.kdsatker and a.thang=c.thang and a.kdjenbel=c.kdjenbel
left join (select a.kdkotama, a.kdsatker, a.thang, sum(a.realisasi) as blnini, b.kdjenbel from realisasi a left join yanmasum b on a.id_pagu=b.id_pagu where a.kdbulan='$_GET[kdbulan]' group by a.kdkotama, a.kdsatker, a.thang, b.kdjenbel) as d on a.kdkotama=d.kdkotama and a.kdsatker=d.kdsatker and a.thang=d.thang and a.kdjenbel=d.kdjenbel
where a.kdkotama='$_GET[kdkotama]' and a.kdsatker='$_GET[kdsatker]' and a.thang='$_GET[thang]' 
...
left join (select kdkotama, kdsatker, thang, kdjenbel, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  from yanmasum where kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdjenbel) as b on a.kdjenbel=b.kdjenbel 
left join (select kdkotama, kdsatker, thang, sum(realisasi) as blnlalu, kdjenbel from realisasi_yanmasum where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdjenbel) as c on a.kdjenbel=c.kdjenbel
left join (select kdkotama, kdsatker, thang, sum(realisasi) as blnini, kdjenbel from realisasi_yanmasum  where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdjenbel) as d on a.kdjenbel=d.kdjenbel where a.kdjenbel<'54' order by a.kdjenbel"); 
$pdf->Ln(); 
 $k=1;

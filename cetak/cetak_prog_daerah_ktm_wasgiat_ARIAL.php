...
$pdf->Cell(12,5,'Tanggal',0,0,'L',1);
	 ...
	 $sql= mysql_query("SELECT '' as id_pagu,  a.kdwasgiat, a.kdkotama,  a.thang, '' as kode, '1' as display,  'RUPIAH MURNI' as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.blnlalu, d.blnini
FROM dipa a 
left join(select kdkotama, kdwasgiat, thang, sum(realisasi) as blnlalu from realisasix  where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdkotama,thang, kdwasgiat) as c on a.kdkotama=c.kdkotama  and a.thang=c.thang and a.kdwasgiat=c.kdwasgiat
left join(select kdkotama, kdwasgiat, thang, sum(realisasi) as blnini from realisasix  where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdkotama,thang, kdwasgiat) as d on a.kdkotama=d.kdkotama and a.thang=d.thang and a.kdwasgiat=d.kdwasgiat
where  a.kdkotama='$_GET[kdkotama]'  and a.thang='$_GET[thang]' and a.kdwasgiat='$_GET[kdwasgiat]'
group by a.kdkotama, a.thang, a.kdwasgiat
...
left join (select a.kdprogram, a.kdkotama, a.kdwasgiat, a.thang, sum(b.realisasi) as blnlalu from dipa a left join realisasix b on a.id_pagu=b.id_pagu where b.kdbulan<'$_GET[kdbulan]' and a.kdkotama='$_GET[kdkotama]' and a.thang='$_GET[thang]' and a.kdwasgiat='$_GET[kdwasgiat]' group by a.kdprogram) as c on  a.kdprogram=c.kdprogram
left join (select a.kdprogram, a.kdkotama,  a.thang, sum(b.realisasi) as blnini from dipa a left join realisasix b on a.id_pagu=b.id_pagu where b.kdbulan='$_GET[kdbulan]' and a.kdkotama='$_GET[kdkotama]' and a.thang='$_GET[thang]' and a.kdwasgiat='$_GET[kdwasgiat]' group by a.kdprogram) as d on  a.kdprogram=d.kdprogram
where  a.kdkotama='$_GET[kdkotama]' and a.thang='$_GET[thang]' and a.kdwasgiat='$_GET[kdwasgiat]'  
...
left join (select kdprogram, kdgiat, kdkotama,  thang, sum(realisasi) as blnlalu from realisasix where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram,kdgiat) as c on  a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat
left join (select kdprogram, kdgiat, kdkotama,  thang, sum(realisasi) as blnini from  realisasix  where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram,kdgiat) as d on  a.kdprogram=d.kdprogram and  a.kdgiat=d.kdgiat
where  a.kdkotama='$_GET[kdkotama]' and a.thang='$_GET[thang]'  and a.kdwasgiat='$_GET[kdwasgiat]' 
...
left join (select kdprogram, kdgiat, kdoutput, kdkotama, kdwasgiat,  thang, sum(realisasi) as blnlalu from realisasix where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram,kdgiat,kdoutput) as c on  a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput
left join (select kdprogram, kdgiat, kdoutput, kdkotama, kdwasgiat, thang, sum(realisasi) as blnini from  realisasix  where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram,kdgiat,kdoutput) as d on  a.kdprogram=d.kdprogram and  a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput
where  a.kdkotama='$_GET[kdkotama]' and a.thang='$_GET[thang]'  and a.kdwasgiat='$_GET[kdwasgiat]' 
...
FROM dipa a 
left join (select kdprogram, kdgiat, kdoutput, kdakun,  kdkotama, thang, sum(realisasi) as blnlalu from realisasix  where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram, kdgiat,kdoutput, kdakun) as c on  a.kdprogram=c.kdprogram and a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput and a.kdakun=c.kdakun 
left join (select kdprogram, kdgiat, kdoutput, kdakun,  kdkotama,  thang, sum(realisasi) as blnini from realisasix  where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram, kdgiat,kdoutput, kdakun) as d on  a.kdprogram=d.kdprogram and a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput and a.kdakun=d.kdakun 
where  a.kdkotama='$_GET[kdkotama]' and a.thang='$_GET[thang]'  and a.kdwasgiat='$_GET[kdwasgiat]' group by  a.kdprogram, a.kdgiat,a.kdoutput,a.kdakun)
...
sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.blnlalu, d.blnini
FROM dipa a 
left join (select kdprogram, kdgiat, kdoutput, kdakun, kdsakun, kdkotama, kdwasgiat, thang, sum(realisasi) as blnlalu from realisasix  where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and  thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdgiat,kdoutput, kdakun,kdsakun) as c on  a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput and a.kdakun=c.kdakun and a.kdsakun=c.kdsakun 
left join (select  kdprogram,  kdgiat, kdoutput, kdakun, kdsakun, kdkotama, kdwasgiat, thang, sum(realisasi) as blnini from realisasix where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and  thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdgiat,kdoutput, kdakun,kdsakun) as d on  a.kdprogram=d.kdprogram and  a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput and a.kdakun=d.kdakun and a.kdsakun=d.kdsakun 
where  a.kdkotama='$_GET[kdkotama]' and a.thang='$_GET[thang]' and a.kdwasgiat='$_GET[kdwasgiat]' group by a.kdprogram, a.kdgiat,a.kdoutput,a.kdakun,a.kdsakun) order by display"); 
...
case "06" : $bidang="PEREENCANAAN";break;
}
...
// memberikan nomor urut berupa romawi, norut dan abjad	
	...
	if ($pj <'18' ) { // jika digit kurang dari 14 dicetak tebal (sampai kode output)
	    $pdf->SetFont('Arial','B',8);
		$pdf->SetY($ng);
		$pdf->SetX(15);
...
left join (select id_pagu,kdkotama,   thang, kdwasgiat, sum(realisasi) as blnlalu from realisasix where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and  thang='$_GET[thang]'  and kdwasgiat='$_GET[kdwasgiat]' group by kdkotama,  thang, kdwasgiat) as c on a.kdkotama=c.kdkotama  and a.thang=c.thang and a.kdwasgiat=c.kdwasgiat
left join (select id_pagu,kdkotama,   thang, kdwasgiat, sum(realisasi) as blnini from realisasix  where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and  thang='$_GET[thang]'  and kdwasgiat='$_GET[kdwasgiat]'  group by kdkotama,  thang, kdwasgiat) as d on a.kdkotama=d.kdkotama  and a.thang=d.thang and a.kdwasgiat=d.kdwasgiat
...
left join (select kdkotama,  thang, sum(realisasi) as blnlalu, kdjenbel from realisasix where kdbulan<'$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdjenbel) as c on  a.kdjenbel=c.kdjenbel 

left join (select kdkotama,  thang, sum(realisasi) as blnini, kdjenbel from realisasix where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdjenbel) as d on 
a.kdjenbel=d.kdjenbel 

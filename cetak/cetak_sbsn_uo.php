...
$pdf->SetY(35);
$pdf->SetFont('Arial','',9);
$pdf->Cell(0,5,'PELAKSANAAN ANGGARAN SBSN',0,1,'C');
$pdf->SetY(40); 
...
from sbsn a 
left join (select id_pagu, thang, sum(realisasi) as blnlalu from realisasi_sbsn  where kdbulan<'$_GET[kdbulan]' and  
thang='$_GET[thang]' group by thang) as c on  a.thang=c.thang 
left join (select id_pagu,   thang, sum(realisasi) as blnini from realisasi_sbsn where kdbulan='$_GET[kdbulan]' and  
...
left join (select kdjenbel,   thang, sum(realisasi) as blnlalu  from realisasi_sbsn where kdbulan<'$_GET[kdbulan]' and   thang='$_GET[thang]' group by  kdjenbel) as c on a.kdjenbel=c.kdjenbel 
left join (select  kdjenbel,  thang, sum(realisasi) as blnini from realisasi_sbsn where kdbulan='$_GET[kdbulan]' and   thang='$_GET[thang]' group by kdjenbel) as d on a.kdjenbel=d.kdjenbel 

...
$pdf->Cell(12,5,'Tanggal',0,0,'L',1);
	 ...
	 $sql= mysql_query("select '' as kode, '0' as display, a.kdsatker, b.nmsatkr as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.sppini, c.spmini, c.realsini
from dipa a 
left join t_satkr b on a.kdsatker=b.kdsatkr
left join (select id_pagu,kdkotama, kdsatker,  thang, kdwasgiat, sum(nilai_spp) as sppini, sum(nilai_spm) as spmini, sum(realisasi) as realsini from  realisasi  where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdkotama, kdsatker, thang,kdwasgiat) as c on a.kdkotama=c.kdkotama and a.kdsatker=c.kdsatker and a.thang=c.thang and a.kdwasgiat=c.kdwasgiat
...
left join (select kdprogram, kdkotama, kdsatker, thang, sum(nilai_spp) as sppini, sum(nilai_spm) as spmini, sum(realisasi) as realsini from realisasi where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' and kdwasgiat='04' group by kdprogram) as c on a.kdprogram=c.kdprogram
where a.kdkotama='$_GET[kdkotama]' and a.kdsatker='$_GET[kdsatker]' and a.thang='$_GET[thang]' and a.kdwasgiat='04'
...
left join (select kdprogram, kdgiat, kdkotama, kdsatker, thang, sum(nilai_spp) as sppini, sum(nilai_spm) as spmini, sum(realisasi) as realsini  from realisasi  where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]'  and kdwasgiat='04' group by kdprogram,kdgiat) as c on  a.kdprogram=c.kdprogram and a.kdgiat=c.kdgiat
where a.kdkotama='$_GET[kdkotama]' and a.kdsatker='$_GET[kdsatker]' and a.thang='$_GET[thang]'  and a.kdwasgiat='04'
...
left join (select kdprogram, kdgiat, kdoutput, kdkotama, kdsatker, thang, sum(nilai_spp) as sppini, sum(nilai_spm) as spmini, sum(realisasi) as realsini from  realisasi  where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]'  and kdwasgiat='04' group by kdprogram,kdgiat,kdoutput) as c on  a.kdprogram=c.kdprogram and a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput 
where a.kdkotama='$_GET[kdkotama]' and a.kdsatker='$_GET[kdsatker]' and a.thang='$_GET[thang]'  and a.kdwasgiat='04'
...
a.pagu, a.revisi, a.pagurevisi,  c.sppini, c.spmini, c.realsini
from dipa a 
left join (select id_pagu, kdprogram, kdgiat, kdoutput, kdakun, kdsakun, kdkotama, kdsatker, thang, sum(nilai_spp) as sppini, sum(nilai_spm) as spmini, sum(realisasi) as realsini from  realisasi  where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]'  and kdwasgiat='04' group by id_pagu) as c on  a.id_pagu=c.id_pagu
where a.kdkotama='$_GET[kdkotama]' and a.kdsatker='$_GET[kdsatker]' and a.thang='$_GET[thang]'  and a.kdwasgiat='04'
...
$pdf->Cell(15,5,'2',1,0,'C',0); 
$pdf->Cell(11,5,'3',1,0,'C',0);
$pdf->Cell(80,5,'4',1,0,'C',0); 
$pdf->Cell(25,5,'5',1,0,'C',0);
$pdf->Cell(25,5,'6',1,0,'C',0);
$pdf->Cell(24,5,'7',1,0,'C',0);
$pdf->Cell(24,5,'8',1,0,'C',0);
$pdf->Cell(24,5,'9',1,0,'C',0);
$pdf->Cell(11,5,'10',1,0,'C',0);
$pdf->Cell(24,5,'11',1,0,'C',0);
$pdf->Cell(24,5,'12',1,0,'C',0);
$pdf->Cell(24,5,'13',1,0,'C',0);
$pdf->Cell(5,5,'14',1,0,'C',0);
...
$pdf->SetY(18); 
$pdf->SetX(8); 
$pdf->Cell(7,12,'NO',1,0,'C',0); 

$pdf->SetY(18); 
$pdf->SetX(15);
$pdf->Cell(15,6,'KODE','LRT',0,'C',0); 
$pdf->SetY(24); 
$pdf->SetX(15);
$pdf->Cell(15,6,'AKUN','LRB',0,'C',0); 

$pdf->SetY(18); 
$pdf->SetX(30);
$pdf->Cell(11,6,'KODE','LRT',0,'C',0); 
$pdf->SetY(24); 
$pdf->SetX(30);
$pdf->Cell(11,6,'SKR','LRB',0,'C',0); 

$pdf->SetY(18); 
$pdf->SetX(41); 
$pdf->Cell(80,12,'PROGRAM/KEGIATAN/OUTPUT/JENIS PEKERJAAN',1,0,'C',0); 

$pdf->SetY(18); 
$pdf->SetX(121);
$pdf->Cell(25,12,'DIPA',1,0,'C',0); 

$pdf->SetY(18); 
$pdf->SetX(146);
$pdf->Cell(25,6,'DIPA SETELAH','LRT',0,'C',0); 
$pdf->SetY(24); 
$pdf->SetX(146);
$pdf->Cell(25,6,'REVISI','LRB',0,'C',0); 

$pdf->SetY(18); 
$pdf->SetX(171);
$pdf->Cell(24,6,'SPP','LRT',0,'C',0); 
$pdf->SetY(24); 
$pdf->SetX(171);
$pdf->Cell(24,6,'S.D. BULAN INI','LRB',0,'C',0); 

$pdf->SetY(18); 
$pdf->SetX(195);
$pdf->Cell(24,6,'SPM','LRT',0,'C',0); 
$pdf->SetY(24); 
$pdf->SetX(195);
$pdf->Cell(24,6,'S.D. BULAN INI','LRB',0,'C',0); 
...
$pdf->SetY(18); 
$pdf->SetX(243);
$pdf->Cell(11,4,'DAYA','LRT',0,'C',0); 
$pdf->SetY(22); 
$pdf->SetX(243);
$pdf->Cell(11,4,'SERAP','LR',0,'C',0); 
$pdf->SetY(26); 
$pdf->SetX(243);
...
$pdf->SetY(18); 
$pdf->SetX(254);
$pdf->Cell(72,4,'SISA ANGGARAN','LRT',0,'C',0); 

$pdf->SetY(22); 
$pdf->SetX(254);
$pdf->Cell(24,4,'BELUM SPP','LRT',0,'C',0); 
$pdf->SetY(26); 
$pdf->SetX(254);
$pdf->Cell(24,4,'S.D. BULAN INI','LRB',0,'C',0);

$pdf->SetY(22); 
$pdf->SetX(278);
$pdf->Cell(24,4,'BELUM SPM','LRT',0,'C',0); 
$pdf->SetY(26); 
$pdf->SetX(278);
$pdf->Cell(24,4,'S.D. BULAN INI','LRB',0,'C',0); 
...
$pdf->SetY(22); 
$pdf->SetX(302);
$pdf->Cell(24,4,'BELUM SP2D','LRT',0,'C',0); 
$pdf->SetY(26); 
$pdf->SetX(302);
$pdf->Cell(24,4,'S.D. BULAN INI','LRB',0,'C',0); 
...
$pdf->SetY(18); 
$pdf->SetX(326);
$pdf->Cell(5,4,'K','LRT',0,'C',0); 
$pdf->SetY(22); 
$pdf->SetX(326);
$pdf->Cell(5,4,'E','LR',0,'C',0); 
$pdf->SetY(26); 
$pdf->SetX(326);
$pdf->Cell(5,4,'T','LRB',0,'C',0); 
...
// memberikan nomor urut berupa romawi, norut dan abjad	
	...
	if ($pj=='2') {	
	    $pdf->SetFont('arialnarrow','',9.5);
		if ($romawi=='1') {
		$pdf->Cell(9,5,'A.','LR',1,'L',2);
        } else if ($romawi=='2') {
		$pdf->Cell(9,5,'B.','LR',1,'L',2);	
		} else if ($romawi=='3') {
		$pdf->Cell(9,5,'C.','LR',1,'L',2);		
		} else if ($romawi=='4') {
		$pdf->Cell(9,5,'D.','LR',1,'L',2);	
		} else if ($romawi=='5') {
		$pdf->Cell(9,5,'E.','LR',1,'L',2);			
		} else {
		$pdf->Cell(9,5,'F.','LR',1,'L',2);
		}
		$tempRomawi = $romawi;
        $romawi++;		
...
left join (select kdkotama, kdsatker, thang, kdjenbel, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  from dipa where kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdjenbel) as b on a.kdjenbel=b.kdjenbel 
left join (select kdkotama, kdsatker, thang, sum(nilai_spp) as sppini, sum(nilai_spm) as spmini, sum(realisasi) as realsini, kdjenbel from realisasi  where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdjenbel) as c on a.kdjenbel=c.kdjenbel 
...
$pdf->Ln(); 
 ...
 while($ok = mysql_fetch_array($jmljenbel)) {
	...
	$prosen_des0= number_format($prosen,0,',','.');
		...
		$pdf->SetY($ng);
		$pdf->SetX(15);
		$pdf->Cell(15,5,$ok[kdjenbel],'LR',1,'C',0);
		
		$pdf->SetY($ng);
		$pdf->SetX(30);
		$pdf->Cell(11,5,'','LR',1,'C',0);
		
		$pdf->SetY($ng);
		$pdf->SetX(41);
		$pdf->Cell(80,5,$ok[nmjenbel],'LR',1,'L',0);
		
		$pdf->SetY($ng);
		$pdf->SetX(121);
		$pdf->Cell(25,5,$pagu,'LR',1,'R',0);
		$pdf->SetY($ng);
		$pdf->SetX(146);
		$pdf->Cell(25,5,$pagurevisi,'LR',1,'R',0);
		
		$pdf->SetY($ng);
		$pdf->SetX(171);
		$pdf->Cell(24,5,$sppini,'LR',1,'R',0);
		
		$pdf->SetY($ng);
		$pdf->SetX(195);
		$pdf->Cell(24,5,$spmini,'LR',1,'R',0);
		$pdf->SetY($ng);
		$pdf->SetX(219);
		$pdf->Cell(24,5,$realsini,'LR',1,'R',0);
			
		$pdf->SetY($ng);
		$pdf->SetX(243);
		 if ($prosen>='100') {	
		$pdf->Cell(11,5,$prosen_des0,'LR',1,'C',0); 
		 } else {
		$pdf->Cell(11,5,$prosen_des,'LR',1,'C',0); 	 
		 }
		 
		$pdf->SetY($ng);
		$pdf->SetX(254);
		$pdf->Cell(24,5,$sisa_spp_rb,'LR',1,'R',0);
		
		$pdf->SetY($ng);
		$pdf->SetX(278);
		$pdf->Cell(24,5,$sisa_spm_rb,'LR',1,'R',0);
		
		$pdf->SetY($ng);
		$pdf->SetX(302);
		$pdf->Cell(24,5,$sisa_reals_rb,'LR',1,'R',0);
		 	 
		$pdf->SetY($ng);
		$pdf->SetX(326);
		$pdf->Cell(5,5,'','LR',1,'R',0); 
	}
	$y_axis1= $y_axis1 + $row_height;
		$k++;
		 $ng = $pdf->GetY(); 
    $pdf->SetX(8); 
    $pdf->Cell(7,1,'','LRB',0,'C',0); 
	$pdf->Cell(15,1,'','LRB',0,'C',0); 
	$pdf->Cell(11,1,'','LRB',0,'C',0);
	$pdf->Cell(80,1,'','LRB',0,'C',0); 
	$pdf->Cell(25,1,'','LRB',0,'C',0);
	$pdf->Cell(25,1,'','LRB',0,'C',0);
	$pdf->Cell(24,1,'','LRB',0,'C',0);
	$pdf->Cell(24,1,'','LRB',0,'C',0);
	$pdf->Cell(24,1,'','LRB',0,'C',0);
	$pdf->Cell(11,1,'','LRB',0,'C',0);
	$pdf->Cell(24,1,'','LRB',0,'C',0);

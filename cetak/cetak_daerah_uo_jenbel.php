...
$pdf->Sety(15); 
$pdf->SetX(250); 
$pdf->Cell(35,5,$z[brs1],0,0,'R',1);

$pdf->Sety(20); 
$pdf->SetX(250); 
$pdf->Cell(35,5,$z[brs2],0,0,'R',1);
$pdf->Sety(25); 
$pdf->SetX(250); 
$pdf->Cell(35,5,$z[brs3],0,0,'R',1);
$pdf->Sety(30.3); 
$pdf->SetX(234.5);
$pdf->Cell($garis,0,'                             ','T',0,'L',1);


$pdf->Sety(15); 
$pdf->SetX(233); 
$pdf->Cell(20,5,'Lampiran'.' '.$_GET['lamp'],0,0,'L',0);
$pdf->Sety(20); 
$pdf->SetX(233); 
$pdf->Cell(20,5,'Nomor ',0,0,'L',0);
$pdf->Sety(25); 
$pdf->SetX(233); 
$pdf->Cell(20,5,'Tanggal',0,0,'L',0);
...
//------------------------------------------- JUMLAH RM ----------------------------	
$jml_daerah=mysql_query("select  a.thang, sum(a.pagu) as pagu,  sum(a.revisi) as revisi, sum(a.pagurevisi)  as pagurevisi, b.realisasi from dipa a
left join (select    thang, sum(realisasi) as realisasi from realisasix where kdbulan<='$_GET[kdbulan]'   and thang='$_GET[thang]' group by   thang) as b on   a.thang=b.thang where a.thang='$_GET[thang]' group by  a.thang"); 
...
left join (select  kdjenbel,   thang, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  from dipa where  thang='$_GET[thang]' group by  kdjenbel)  as b on   a.kdjenbel=b.kdjenbel
left join (select id_pagu, kdjd, kdjenbel,  thang, sum(realisasi) as realisasi from realisasix where  thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as c on a.kdjenbel=c.kdjenbel
...
$jml_yanmasum=mysql_query("select a.thang, sum(a.pagu) as pagu,  sum(a.revisi) as revisi, sum(a.pagurevisi)  as pagurevisi, b.realisasi from yanmasum a
left join (select   thang, sum(realisasi) as realisasi  from realisasi_yanmasum where kdbulan<='$_GET[kdbulan]' and  thang='$_GET[thang]' group by  thang) as b on  a.thang=b.thang
where  a.thang='$_GET[thang]' group by  a.thang"); 
...
b.pagu,  b.revisi, b.pagurevisi, c.realisasi  from t_jenbel a 
left join (select  kdjenbel,   thang, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  from yanmasum where   thang='$_GET[thang]' group by  kdjenbel)  as b on   a.kdjenbel=b.kdjenbel
left join (select id_pagu, kdjd, kdjenbel,  thang, sum(realisasi) as realisasi from  realisasi_yanmasum  where   thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as c on a.kdjenbel=c.kdjenbel group by a.kdjenbel order by a.kdjenbel"); 	
...
$jml_bpjs=mysql_query("select   a.thang, sum(a.pagu) as pagu,  sum(a.revisi) as revisi, sum(a.pagurevisi)  as pagurevisi, b.realisasi from bpjs a
left join (select thang, sum(realisasi) as realisasi  from realisasi_bpjs  where kdbulan<='$_GET[kdbulan]'  and thang='$_GET[thang]' group by thang) as b on a.thang=b.thang
where    a.thang='$_GET[thang]' group by   a.thang"); 
...
b.pagu,  b.revisi, b.pagurevisi, c.realisasi  from t_jenbel a 
left join (select  kdjenbel,  thang, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  from bpjs where     thang='$_GET[thang]' group by  kdjenbel)  as b on   a.kdjenbel=b.kdjenbel
left join (select id_pagu, kdjd, kdjenbel,   thang, sum(realisasi) as realisasi from realisasi_bpjs where   thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as c on a.kdjenbel=c.kdjenbel
group by a.kdjenbel order by a.kdjenbel"); 	
...
$jml_blu=mysql_query("select a.thang, sum(a.pagu) as pagu,  sum(a.revisi) as revisi, sum(a.pagurevisi)  as pagurevisi, b.realisasi from blu a
left join (select  thang, sum(realisasi) as realisasi  from realisasi_blu  where kdbulan<='$_GET[kdbulan]' and thang='$_GET[thang]' group by  thang) as b on   a.thang=b.thang
where   a.thang='$_GET[thang]' group by  a.thang"); 
...
b.pagu,  b.revisi, b.pagurevisi, c.realisasi  from t_jenbel a 
left join (select  kdjenbel,   kdsatker, thang, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  from blu where   thang='$_GET[thang]' group by  kdjenbel)  as b on   a.kdjenbel=b.kdjenbel
left join (select id_pagu, kdjd, kdjenbel,  kdsatker, thang, sum(realisasi) as realisasi from realisasi_blu where thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as c on a.kdjenbel=c.kdjenbel
group by a.kdjenbel order by a.kdjenbel"); 	
...
$jml_hibah=mysql_query("select  a.thang, sum(a.pagu) as pagu,  sum(a.revisi) as revisi, sum(a.pagurevisi)  as pagurevisi, b.realisasi from hibah a
left join (select thang, sum(realisasi) as realisasi  from realisasi_hibah where kdbulan<='$_GET[kdbulan]' and thang='$_GET[thang]' group by   thang) as b on a.thang=b.thang
...
b.pagu,  b.revisi, b.pagurevisi, c.realisasi  from t_jenbel a 
left join (select  kdjenbel,  thang, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  from hibah where  thang='$_GET[thang]' group by  kdjenbel)  as b on   a.kdjenbel=b.kdjenbel
left join (select id_pagu, kdjd, kdjenbel,  thang, sum(realisasi) as realisasi from  realisasi_hibah where thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as c on a.kdjenbel=c.kdjenbel
group by a.kdjenbel
order by a.kdjenbel"); 	
...
$jml_kapitasi=mysql_query("select  a.thang, sum(a.pagu) as pagu,  sum(a.revisi) as revisi, sum(a.pagurevisi)  as pagurevisi, b.realisasi from kapitasi a
left join (select  thang, sum(realisasi) as realisasi  from realisasi_kapitasi  where kdbulan<='$_GET[kdbulan]'  and thang='$_GET[thang]' group by   thang) as b on  a.thang=b.thang
where   a.thang='$_GET[thang]'
group by a.thang"); 
...
b.pagu,  b.revisi, b.pagurevisi, c.realisasi  from t_jenbel a 
left join (select  kdjenbel,    thang, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  from kapitasi where   thang='$_GET[thang]' group by  kdjenbel)  as b on   a.kdjenbel=b.kdjenbel
left join (select id_pagu, kdjd, kdjenbel,   thang, sum(realisasi) as realisasi from realisasi_kapitasi   where   thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as c on a.kdjenbel=c.kdjenbel
group by a.kdjenbel
order by a.kdjenbel"); 	
...
from sbsn a
left join (select   thang, sum(realisasi) as realisasi  from realisasi_sbsn  where kdbulan<='$_GET[kdbulan]' and    thang='$_GET[thang]' group by   thang) as b on  a.thang=b.thang
where   a.thang='$_GET[thang]' group by  a.thang"); 
...
b.pagu,  b.revisi, b.pagurevisi, c.realisasi  from t_jenbel a 
left join (select  kdjenbel,    thang, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  from sbsn where   thang='$_GET[thang]' group by  kdjenbel)  as b on   a.kdjenbel=b.kdjenbel
left join (select id_pagu, kdjd, kdjenbel,  thang, sum(realisasi) as realisasi from realisasi_sbsn   where  thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as c on a.kdjenbel=c.kdjenbel
group by a.kdjenbel
order by a.kdjenbel"); 	
...
$perjenbel_global=mysql_query("select w.kdjenbel, w.nmjenbel,   z.thang, z.pagu, z.revisi, z.pagurevisi, z.realisasi from t_jenbel w left join (select   z.thang, z.kdjenbel, sum(z.pagu) as pagu, sum(z.revisi) as revisi, sum(z.pagurevisi) as pagurevisi, sum(z.realisasi) as realisasi 
from 

(select  'dipa' as lap, a.kdjenbel, a.thang, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, b.realisasi  from dipa a 
left join (select id_pagu, kdjd, kdjenbel,   thang, sum(realisasi) as realisasi from  realisasix where  thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as b on   a.kdjenbel=b.kdjenbel
where  a.thang='$_GET[thang]' group by  a.kdjenbel

union
(select   'yanmasum' as lap, a.kdjenbel, a.thang, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, b.realisasi  from yanmasum a 
left join (select id_pagu, kdjd, kdjenbel, thang, sum(realisasi) as realisasi from  realisasi_yanmasum where thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as b on   a.kdjenbel=b.kdjenbel
where a.thang='$_GET[thang]' group by  a.kdjenbel)

union
(select   'bpjs' as lap, a.kdjenbel,   a.thang, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, b.realisasi  from bpjs a 
left join (select id_pagu, kdjd, kdjenbel,  thang, sum(realisasi) as realisasi from  realisasi_bpjs where   thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as b on   a.kdjenbel=b.kdjenbel
where    a.thang='$_GET[thang]' group by  a.kdjenbel)

union
(select   'blu' as lap, a.kdjenbel,    a.thang, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, b.realisasi  from blu a 
left join (select id_pagu, kdjd, kdjenbel,  thang, sum(realisasi) as realisasi from  realisasi_blu where   thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as b on   a.kdjenbel=b.kdjenbel
where  a.thang='$_GET[thang]' group by  a.kdjenbel)

union
(select   'kapitasi' as lap, a.kdjenbel,   a.thang, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, b.realisasi  from kapitasi a 
left join (select id_pagu, kdjd, kdjenbel,   thang, sum(realisasi) as realisasi from  realisasi_kapitasi where   thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as b on   a.kdjenbel=b.kdjenbel
where   a.thang='$_GET[thang]' group by  a.kdjenbel)

union
(select   'hibah' as lap, a.kdjenbel,    a.thang, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, b.realisasi  from hibah a 
left join (select id_pagu, kdjd, kdjenbel,   thang, sum(realisasi) as realisasi from  realisasi_hibah where thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as b on   a.kdjenbel=b.kdjenbel
where    a.thang='$_GET[thang]' group by  a.kdjenbel)

union
(select   'sbsn' as lap, a.kdjenbel,    a.thang, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, b.realisasi  from sbsn a 
left join (select id_pagu, kdjd, kdjenbel,   thang, sum(realisasi) as realisasi from  realisasi_sbsn where thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as b on   a.kdjenbel=b.kdjenbel
where    a.thang='$_GET[thang]' group by  a.kdjenbel)

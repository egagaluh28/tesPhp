...
from sbsn a
left join (select kdkotama, kdsatker, thang, sum(realisasi) as realisasi  from realisasi_sbsn where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by kdkotama, kdsatker, thang) as b on a.kdkotama=b.kdkotama and a.kdsatker=b.kdsatker and a.thang=b.thang
where  a.kdkotama='$_GET[kdkotama]' and a.kdsatker='$_GET[kdsatker]' and a.thang='$_GET[thang]'
group by a.kdkotama, a.kdsatker, a.thang"); 
...
b.pagu,  b.revisi, b.pagurevisi, c.realisasi  from t_jenbel a 
left join (select  kdjenbel, kdkotama, kdsatker, thang, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  from sbsn where  kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' group by  kdjenbel)  as b on   a.kdjenbel=b.kdjenbel
left join (select id_pagu, kdjd, kdjenbel, kdkotama, kdsatker, thang, sum(realisasi) as realisasi from realisasi_sbsn where kdkotama='$_GET[kdkotama]' and kdsatker='$_GET[kdsatker]' and thang='$_GET[thang]' and kdbulan<='$_GET[kdbulan]' group by kdjenbel) as c on a.kdjenbel=c.kdjenbel
group by a.kdjenbel order by a.kdjenbel"); 	
...
union
(select  'sbsn' as lap, a.kdjenbel, a.kdkotama, a.kdsatker, a.thang, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, b.realisasi  from sbsn a 
left join (select a.id_pagu, a.kdjd, a.kdjenbel, a.kdkotama, a.kdsatker, a.thang, sum(b.realisasi) as realisasi from sbsn a left join realisasi_sbsn b on a.id_pagu=b.id_pagu where a.kdkotama='$_GET[kdkotama]' and a.kdsatker='$_GET[kdsatker]' and a.thang='$_GET[thang]' and b.kdbulan<='$_GET[kdbulan]' group by a.kdjenbel) as b on   a.kdjenbel=b.kdjenbel

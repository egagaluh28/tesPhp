...
$pdf->Cell(12,5,'Tanggal',0,0,'L',1);
	 ...
	 $sql= mysql_query("SELECT '' as pengembalian, '' as id_pagu,  a.kdwasgiat, a.kdkotama,  a.thang, '' as kode, '1' as display,  'RUPIAH MURNI' as uraian, sum(a.pagurevisi) as pagurevisi,  '' as id_realisasi, c.realisasi, c1.realisasi2, d.jml_pengembalian
FROM dipa a 
left join(select kdkotama,  thang, sum(realisasi) as realisasi from realisasi where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' and pengembalian='x' group by kdkotama,thang) as c on a.kdkotama=c.kdkotama   and a.thang=c.thang 
left join(select kdkotama,  thang, sum(realisasi) as realisasi2 from realisasi where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' and pengembalian='x' group by kdkotama,thang) as c1 on a.kdkotama=c1.kdkotama and   a.thang=c1.thang 
left join(select kdkotama,  thang, sum(jml_pengembalian) as jml_pengembalian from pengembalian where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' group by kdkotama,thang) as d on a.kdkotama=d.kdkotama and   a.thang=d.thang 
where  a.kdkotama='$_GET[kdkotama]'  and a.thang='$_GET[thang]' and a.pengembalian='x' 
...
left join (select kdprogram, kdkotama,  thang, sum(realisasi) as realisasi from realisasi where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' and pengembalian='x' group by kdprogram) as c on a.kdprogram=c.kdprogram
left join (select kdprogram, kdkotama, thang, sum(realisasi) as realisasi2 from realisasi where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' and pengembalian='x' group by kdprogram) as c1 on a.kdprogram=c1.kdprogram
left join (select kdprogram, kdkotama, thang, sum(jml_pengembalian) as jml_pengembalian from pengembalian where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' group by kdprogram) as d on a.kdprogram=d.kdprogram
where  a.kdkotama='$_GET[kdkotama]'  and a.thang='$_GET[thang]' and a.pengembalian='x' group by a.kdprogram)
...
where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' and pengembalian='x' group by kdprogram,kdgiat) as c on a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat
left join (select kdprogram, kdgiat, kdkotama,  thang, sum(realisasi) as realisasi2 from realisasi 
where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' and pengembalian='x' group by kdprogram,kdgiat) as c1 on a.kdprogram=c1.kdprogram and  a.kdgiat=c1.kdgiat
left join (select kdprogram, kdgiat, kdkotama,  thang, sum(jml_pengembalian) as jml_pengembalian from pengembalian where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' group by kdprogram,kdgiat) as d on a.kdprogram=d.kdprogram and  d.kdgiat=d.kdgiat
where  a.kdkotama='$_GET[kdkotama]'  and a.thang='$_GET[thang]'  and a.pengembalian='x' group by a.kdprogram,a.kdgiat)
...
where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'   and thang='$_GET[thang]' and pengembalian='x' group by kdprogram,kdgiat,kdoutput) as c on a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput
left join (select kdprogram, kdgiat, kdoutput, kdkotama,  thang, sum(realisasi) as realisasi2 from realisasi 
where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'  and thang='$_GET[thang]' and pengembalian='x' group by kdprogram,kdgiat,kdoutput) as c1 on a.kdprogram=c1.kdprogram and  a.kdgiat=c1.kdgiat and a.kdoutput=c1.kdoutput
...
where  a.kdkotama='$_GET[kdkotama]'   and a.thang='$_GET[thang]'  and a.pengembalian='x' group by a.kdprogram,a.kdgiat,a.kdoutput)
...
FROM dipa a 
left join (select kdgiat, kdoutput, kdakun,  kdkotama,   thang, sum(realisasi) as realisasi from realisasi where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]' and thang='$_GET[thang]' and pengembalian='x' group by kdgiat,kdoutput, kdakun) as c on  a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput and a.kdakun=c.kdakun 
left join (select kdgiat, kdoutput, kdakun,  kdkotama,   thang, sum(realisasi) as realisasi2 from realisasi where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'   and thang='$_GET[thang]' and pengembalian='x' group by kdgiat,kdoutput, kdakun) as c1 on  a.kdgiat=c1.kdgiat and a.kdoutput=c1.kdoutput and a.kdakun=c1.kdakun
left join (select kdgiat, kdoutput, kdakun,  kdkotama,   thang, sum(jml_pengembalian) as jml_pengembalian from pengembalian where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'   and thang='$_GET[thang]' group by kdgiat,kdoutput, kdakun) as d on  a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput and a.kdakun=d.kdakun 
where  a.kdkotama='$_GET[kdkotama]' and   a.thang='$_GET[thang]'  and a.pengembalian='x' group by  a.kdgiat,a.kdoutput,a.kdakun)
...
where  a.kdkotama='$_GET[kdkotama]'  and a.thang='$_GET[thang]'  and a.pengembalian='x' group by  a.kdgiat,a.kdoutput,a.kdakun,a.kdsakun) order by display"); 
...
$jml=mysql_query("SELECT   a.kdwasgiat, a.kdkotama,   a.thang, '' as kode, '1' as display,  sum(a.pagurevisi) as pagurevisi,  '' as id_realisasi, c.realisasi, c1.realisasi2, d.jml_pengembalian
FROM dipa a 
left join(select kdkotama,   thang, sum(realisasi) as realisasi from realisasi where kdbulan='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'   and thang='$_GET[thang]' and pengembalian='x' group by kdkotama,thang) as c on a.kdkotama=c.kdkotama and   a.thang=c.thang 
left join(select kdkotama,   thang, sum(realisasi) as realisasi2 from realisasi where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'   and thang='$_GET[thang]' and pengembalian='x' group by kdkotama, thang) as c1 on a.kdkotama=c1.kdkotama   and a.thang=c1.thang 
left join(select kdkotama,   thang, sum(jml_pengembalian) as jml_pengembalian from pengembalian where kdbulan<='$_GET[kdbulan]' and kdkotama='$_GET[kdkotama]'   and thang='$_GET[thang]' group by kdkotama, thang) as d on a.kdkotama=d.kdkotama and    a.thang=d.thang 
...
//	$netto = $last[realisasi2] + $last[jml_pengembalian];
//	$netto_rb = number_format($netto,0,',','.');
//	
//	if (($netto=='') or ($netto=='0')) {
//	$prosen     = 0; 
...
//	$prosen_des0= number_format($prosen,0,',','.');
	...
	$jturahan = $stlhrevisi - $last[realisasi2];

...
//$kop= mysql_query("select * from kopstuk where kdkotama='00' and kdsatker='00'"); 
...
$pdf->Sety(18); 
$pdf->SetX($posisi+40); 
$pdf->Cell(12,5,'Lampiran',0,0,'L',1);
$pdf->Sety(23); 
$pdf->SetX($posisi+40); 
...
$pdf->Cell(12,5,'Tanggal',0,0,'L',1);
	 ...
	 $sql= mysql_query("SELECT '' as id_pagu,  a.kdwasgiat,   a.thang, '' as kode, '1' as display,  'RUPIAH MURNI' as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.blnlalu, d.blnini
FROM dipa a 
left join(select   kdwasgiat, thang, sum(realisasi) as blnlalu from realisasix  where kdbulan<'01' and  thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by  thang, kdwasgiat) as c on  a.thang=c.thang and a.kdwasgiat=c.kdwasgiat
left join(select   kdwasgiat, thang, sum(realisasi) as blnini from realisasix  where kdbulan='$_GET[kdbulan]' and  thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by  thang, kdwasgiat) as d on  a.thang=d.thang and a.kdwasgiat=d.kdwasgiat
where   a.thang='$_GET[thang]' and a.kdwasgiat='$_GET[kdwasgiat]'
group by   a.thang, a.kdwasgiat

union (SELECT '' as id_pagu, a.kdwasgiat,    a.thang, concat('012.','22.',a.kdprogram)  as kode, concat('1',a.kdprogram) as display, b.nmprogram as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.blnlalu, d.blnini
FROM dipa a 
left join t_program b on a.kdprogram=b.kdprogram
left join (select a.kdprogram,  a.kdwasgiat, a.thang, sum(b.realisasi) as blnlalu from dipa a left join realisasix b on a.id_pagu=b.id_pagu where b.kdbulan<'01'   and a.thang='$_GET[thang]' and a.kdwasgiat='$_GET[kdwasgiat]' group by a.kdprogram) as c on  a.kdprogram=c.kdprogram
left join (select a.kdprogram,    a.thang, sum(b.realisasi) as blnini from dipa a left join realisasix b on a.id_pagu=b.id_pagu where b.kdbulan='$_GET[kdbulan]'   and a.thang='$_GET[thang]' and a.kdwasgiat='$_GET[kdwasgiat]' group by a.kdprogram) as d on  a.kdprogram=d.kdprogram
where    a.thang='$_GET[thang]' and a.kdwasgiat='$_GET[kdwasgiat]'  
group by a.kdprogram)

union (SELECT '' as id_pagu, a.kdwasgiat,    a.thang, a.kdgiat as kode, concat('1',a.kdprogram,a.kdgiat) as display, b.nmgiat as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.blnlalu, d.blnini
FROM dipa a 
left join t_giat b on a.kdgiat=b.kdgiat
left join (select kdprogram, kdgiat,  thang, sum(realisasi) as blnlalu from realisasix where kdbulan<'01' and   thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram,kdgiat) as c on  a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat
left join (select kdprogram, kdgiat,   thang, sum(realisasi) as blnini from  realisasix  where kdbulan='$_GET[kdbulan]' and   thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram,kdgiat) as d on  a.kdprogram=d.kdprogram and  a.kdgiat=d.kdgiat
where   a.thang='$_GET[thang]'  and a.kdwasgiat='$_GET[kdwasgiat]' 
group by a.kdprogram,a.kdgiat)

union (SELECT '' as id_pagu, a.kdwasgiat,   a.thang, a.kdoutput as kode, concat('1',a.kdprogram,a.kdgiat,a.kdoutput) as display, b.nmoutput as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.blnlalu, d.blnini
FROM dipa a 
left join t_output b on a.kdprogram=b.kdprogram and a.kdgiat=b.kdgiat and a.kdoutput=b.kdoutput
left join (select kdprogram, kdgiat, kdoutput,  kdwasgiat,  thang, sum(realisasi) as blnlalu from realisasix where kdbulan<'01'   and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram,kdgiat,kdoutput) as c on  a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput
left join (select kdprogram, kdgiat, kdoutput,   kdwasgiat, thang, sum(realisasi) as blnini from  realisasix  where kdbulan='$_GET[kdbulan]'   and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram,kdgiat,kdoutput) as d on  a.kdprogram=d.kdprogram and  a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput
where   a.thang='$_GET[thang]'  and a.kdwasgiat='$_GET[kdwasgiat]' 
group by a.kdprogram,a.kdgiat,a.kdoutput)

union (SELECT '' as id_pagu,  a.kdwasgiat,   a.thang, a.kdakun as kode, concat('1',a.kdprogram,a.kdgiat,a.kdoutput,a.kdakun) as display, a.nmakun as uraian,  
sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.blnlalu, d.blnini
FROM dipa a 
left join (select kdprogram, kdgiat, kdoutput, kdakun,   thang, sum(realisasi) as blnlalu from realisasix  where kdbulan<'01'  and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram, kdgiat,kdoutput, kdakun) as c on  a.kdprogram=c.kdprogram and a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput and a.kdakun=c.kdakun 
left join (select kdprogram, kdgiat, kdoutput, kdakun,     thang, sum(realisasi) as blnini from realisasix  where kdbulan='$_GET[kdbulan]'  and thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdprogram, kdgiat,kdoutput, kdakun) as d on  a.kdprogram=d.kdprogram and a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput and a.kdakun=d.kdakun 
where   a.thang='$_GET[thang]'  and a.kdwasgiat='$_GET[kdwasgiat]' group by  a.kdprogram, a.kdgiat,a.kdoutput,a.kdakun)

union (SELECT '' as id_pagu,  a.kdwasgiat,  a.thang, '' as kode, concat('1',a.kdprogram,a.kdgiat,a.kdoutput,a.kdakun, a.kdsakun) as display, concat('- ',a.nmsakun) as uraian,  
sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi, c.blnlalu, d.blnini
FROM dipa a 
left join (select kdprogram, kdgiat, kdoutput, kdakun, kdsakun,  kdwasgiat, thang, sum(realisasi) as blnlalu from realisasix  where kdbulan<'01'  and  thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdgiat,kdoutput, kdakun,kdsakun) as c on  a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput and a.kdakun=c.kdakun and a.kdsakun=c.kdsakun 
left join (select  kdprogram,  kdgiat, kdoutput, kdakun, kdsakun,   kdwasgiat, thang, sum(realisasi) as blnini from realisasix where kdbulan='$_GET[kdbulan]'   and  thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdgiat,kdoutput, kdakun,kdsakun) as d on  a.kdprogram=d.kdprogram and  a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput and a.kdakun=d.kdakun and a.kdsakun=d.kdsakun 
where   a.thang='$_GET[thang]' and a.kdwasgiat='$_GET[kdwasgiat]' group by a.kdprogram, a.kdgiat,a.kdoutput,a.kdakun,a.kdsakun) order by display"); 
...
$pdf->SetY(35);
$pdf->SetFont('Arial','',9);
$pdf->Cell(0,5,'PELAKSANAAN ANGGARAN DIPA PETIKAN SATKER BIDANG'.' '.$bidang,0,1,'C');
$pdf->SetY(40); 
...
left join (select id_pagu,kdkotama, thang, sum(realisasi) as blnlalu from realisasix  where  kdbulan<'$_GET[kdbulan]' and  
thang='$_GET[thang]' and  kdwasgiat='$_GET[kdwasgiat]' group by  thang, kdwasgiat) as c on a.kdkotama=c.kdkotama and a.thang=c.thang 
left join (select id_pagu,kdkotama,  thang, sum(realisasi) as blnini from realisasix   where  kdbulan='$_GET[kdbulan]'  and  
thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by  thang, kdwasgiat) as d on a.kdkotama=d.kdkotama and a.thang=d.thang 
where  a.thang='$_GET[thang]'  and a.kdwasgiat='$_GET[kdwasgiat]' 
group by a.thang, a.kdwasgiat"); 
...
left join (select    thang, kdjenbel, sum(pagu) as pagu, sum(revisi) as revisi, sum(pagurevisi) as pagurevisi  from dipa where  thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdjenbel) as b on a.kdjenbel=b.kdjenbel 
left join (select   thang, sum(realisasi) as blnlalu, kdjenbel from realisasix where kdbulan<'$_GET[kdbulan]' and   thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdjenbel) as c on  a.kdjenbel=c.kdjenbel 
left join (select    thang, sum(realisasi) as blnini, kdjenbel from realisasix where kdbulan='$_GET[kdbulan]' and   thang='$_GET[thang]' and kdwasgiat='$_GET[kdwasgiat]' group by kdjenbel) as d on 
a.kdjenbel=d.kdjenbel where a.kdjenbel<'54' order by a.kdjenbel"); 
$pdf->Ln(); 
 $k=1;

...
$query= "SELECT '' as id_pengembalian, '' as pengembalian, '' as id_pagu,  a.kdwasgiat, a.kdkotama, a.kdsatker, a.thang, '' as kode, '1' as display,  'RUPIAH MURNI' as uraian, sum(a.pagurevisi) as pagurevisi,  '' as id_realisasi, c.realisasi, c1.realisasi2, d.jml_pengembalian
FROM dipa a 
left join(select kdkotama, kdsatker, thang, sum(realisasi) as realisasi from realisasi where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' and pengembalian='x' group by kdkotama,kdsatker,thang) as c on a.kdkotama=c.kdkotama and a.kdsatker=c.kdsatker and a.thang=c.thang 
...
where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]' and a.pengembalian='x' 
group by a.kdkotama, a.kdsatker, a.thang
union (SELECT '' as id_pengembalian, '' as pengembalian, '' as id_pagu, a.kdwasgiat, a.kdkotama, a.kdsatker,  a.thang, concat('012.','22.',a.kdprogram) as kode, concat('1',a.kdprogram) as display, b.nmprogram as uraian,  sum(a.pagurevisi) as pagurevisi, '' as id_realisasi,  c.realisasi, c1.realisasi2, d.jml_pengembalian
FROM dipa a 
left join t_program b on a.kdprogram=b.kdprogram
left join (select kdprogram, kdkotama, kdsatker, thang, sum(realisasi) as realisasi from realisasi where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' and pengembalian='x' group by kdprogram) as c on a.kdprogram=c.kdprogram
left join (select kdprogram, kdkotama, kdsatker, thang, sum(realisasi) as realisasi2 from realisasi where kdbulan<='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' and pengembalian='x' group by kdprogram) as c1 on a.kdprogram=c1.kdprogram
left join (select kdprogram, kdkotama, kdsatker, thang, sum(jml_pengembalian) as jml_pengembalian from pengembalian where kdbulan<='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by kdprogram) as d on a.kdprogram=d.kdprogram
where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]' and a.pengembalian='x' group by a.kdprogram)
union (SELECT '' as id_pengembalian, '' as pengembalian, '' as id_pagu, a.kdwasgiat, a.kdkotama, a.kdsatker, a.thang, a.kdgiat as kode, concat('1',a.kdprogram,a.kdgiat) as display, b.nmgiat as uraian,  sum(a.pagurevisi) as pagurevisi,  '' as id_realisasi,  c.realisasi, c1.realisasi2, d.jml_pengembalian 
FROM dipa a 
...
where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' and pengembalian='x' group by kdprogram,kdgiat) as c on a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat
left join (select kdprogram, kdgiat, kdkotama, kdsatker, thang, sum(realisasi) as realisasi2 from realisasi 
where kdbulan<='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' and pengembalian='x' group by kdprogram,kdgiat) as c1 on a.kdprogram=c1.kdprogram and  a.kdgiat=c1.kdgiat
...
where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]'  and a.pengembalian='x' group by a.kdprogram,a.kdgiat)

union (SELECT '' as id_pengembalian, '' as pengembalian, '' as id_pagu, a.kdwasgiat, a.kdkotama, a.kdsatker, a.thang, a.kdoutput as kode, concat('1',a.kdprogram,a.kdgiat,a.kdoutput) as display, b.nmoutput as uraian,  sum(a.pagurevisi) as pagurevisi,  '' as id_realisasi,  c.realisasi, c1.realisasi2, d.jml_pengembalian 
FROM dipa a 
...
where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' and pengembalian='x' group by kdprogram,kdgiat,kdoutput) as c on a.kdprogram=c.kdprogram and  a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput
left join (select kdprogram, kdgiat, kdoutput, kdkotama, kdsatker, thang, sum(realisasi) as realisasi2 from realisasi 
where kdbulan<='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' and pengembalian='x' group by kdprogram,kdgiat,kdoutput) as c1 on a.kdprogram=c1.kdprogram and  a.kdgiat=c1.kdgiat and a.kdoutput=c1.kdoutput
...
where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]'  and a.pengembalian='x' group by a.kdprogram,a.kdgiat,a.kdoutput)

union (SELECT '' as id_pengembalian, '' as pengembalian, '' as id_pagu,  a.kdwasgiat, a.kdkotama, a.kdsatker, a.thang, a.kdakun as kode, concat('1',a.kdprogram,a.kdgiat,a.kdoutput,a.kdakun) as display, a.nmakun as uraian,   sum(a.pagurevisi) as pagurevisi,  '' as id_realisasi,   c.realisasi, c1.realisasi2, d.jml_pengembalian 
FROM dipa a 
...
where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]'  and a.pengembalian='x' group by  a.kdgiat,a.kdoutput,a.kdakun)

union (SELECT '' as id_pengembalian, '' as pengembalian, '' as id_pagu,  a.kdwasgiat, a.kdkotama, a.kdsatker, a.thang, a.kdakun as kode, concat('1',a.kdprogram,a.kdgiat,a.kdoutput,a.kdakun, a.kdsakun) as display, concat('> ',a.nmsakun) as uraian,  
...
where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]'  and a.pengembalian='x' group by  a.kdgiat,a.kdoutput,a.kdakun,a.kdsakun)

union (SELECT d.id_pengembalian, a.pengembalian, a.id_pagu, a.kdwasgiat, a.kdkotama, a.kdsatker, a.thang, '' as kode, concat('1',a.kdprogram,a.kdgiat,a.kdoutput,a.kdakun, a.kdsakun,a.urutitem) as display, concat('-',' ', a.nmitem) as uraian,  sum(a.pagurevisi) as pagurevisi,  c.id_realisasi, c.realisasi, c1.realisasi2, d.jml_pengembalian 
FROM dipa  a
left join (select  id_pagu, id_realisasi, sum(realisasi) as realisasi from realisasi where kdbulan='$_GET[kdbulan]'  and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' and pengembalian='x' group by id_pagu) as c on a.id_pagu=c.id_pagu
left join (select  id_pagu, id_realisasi, sum(realisasi) as realisasi2 from realisasi where kdbulan<='$_GET[kdbulan]'  and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' and pengembalian='x' group by id_pagu) as c1 on a.id_pagu=c1.id_pagu
left join (select  id_pagu, id_realisasi, id_pengembalian, sum(jml_pengembalian) as jml_pengembalian from pengembalian where kdbulan<='$_GET[kdbulan]'  and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]'  group by id_pagu) as d on a.id_pagu=d.id_pagu
where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]'  and a.pengembalian='x'  group by a.id_pagu order by a.noitem) order by display";
	 ...
	 print "<td  valign='top' align='right'></td>";
		  ...
		  print "<td  valign='top' >"; 
				...
				print	"<td  valign='top' align='center'>"; 
					...
					if ($pj>'20')
					print "<a href='media.php?module=inputpengembalian&id_pagu=$k[id_pagu]&id_realisasi=$k[id_realisasi]&kdbulan=$_GET[kdbulan]&thang=$_GET[thang]' data-tooltip='Input Pengembalian' data-position='top' class='top'><img src='images/p.png' width='20' ></a>"; 
					else print "";
				print "</td>";	

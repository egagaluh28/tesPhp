...
//        pakai ini
            ...
            $reload = "media.php?module=realisasi_akuncovid_blu&thang=$_GET[thang]&pagination=true&keyword=$keyword";
...
left join (select  id_pagu, kdakun,  kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi_blu where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' and kdakun in ('525155','537122','537125','525152','525153','525154') group by id_pagu) as c on   a.id_pagu=c.id_pagu 

left join (select  id_pagu, kdakun,  kdkotama, kdsatker, thang, sum(realisasi) as  blnini from realisasi_blu where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' and kdakun in ('525155','537122','537125','525152','525153','525154')  group by id_pagu) as d on   a.id_pagu=d.id_pagu 

where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]' and a.kdakun in ('525155','537122','537125','525152','525153','525154')   group by a.id_pagu) as z 
			WHERE z.kdakun LIKE '%$keyword%' or z.nmitem LIKE '%$keyword%' ORDER BY z.id_pagu");
			
        }else{
//            jika tidak ada pencarian pakai ini
            $reload = "media.php?module=realisasi_akuncovid_blu&thang=$_GET[thang]&pagination=true";
...
where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]' and a.kdakun in ('525155','537122','537125','525152','525153','525154')   group by a.id_pagu order by a.id_pagu");
...
<table  width='98%' align='center'>
    <tr>
		 ...
		 <form name='form1' method='GET'  action='realisasiblu/kirimparameter_akuncovid.php' >
	...
	print "&nbsp;&nbsp;<a href='cetak/cetak_akuncovid_blu.php?kdbulan=$_GET[kdbulan]&thang=$_GET[thang]&kdkotama=$_SESSION[kdkotama]&kdsatker=$_SESSION[kdsatker]' target='_blank' style='text-decoration:none'><input  type='button' value='Cetak' class='button2'/></a>";
	?> 
		...
		<td align="right">
			...
			<a href="<?php print "media.php?module=realisasi_akuncovid_blu&thang=$_GET[thang]&kdbulan=$_GET[kdbulan]"; ?>"

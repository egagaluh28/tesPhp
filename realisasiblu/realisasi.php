...
$query= "SELECT  '' as id_pagu,  a.kdwasgiat, a.kdkotama, a.kdsatker, a.thang, '' as kode, '1' as display,  'BLU' as uraian, sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi,  '' as id_realisasi, c.blnlalu, d.blnini
FROM blu a 
left join(select kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi_blu where kdbulan<'$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by kdkotama,kdsatker,thang) as c on a.kdkotama=c.kdkotama and a.kdsatker=c.kdsatker and a.thang=c.thang 
left join(select kdkotama, kdsatker, thang, sum(realisasi)  as blnini from realisasi_blu where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by kdkotama,kdsatker,thang) as d on a.kdkotama=d.kdkotama and a.kdsatker=d.kdsatker and a.thang=d.thang
...
left join (select kdprogram, kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi_blu where kdbulan<'$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by kdprogram) as c on a.kdprogram=c.kdprogram
left join (select kdprogram, kdkotama, kdsatker, thang, sum(realisasi) as  blnini from realisasi_blu where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by kdprogram) as d on a.kdprogram=d.kdprogram
where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]' 
...
FROM blu a 
left join (select kdgiat, kdoutput, kdakun,  kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi_blu where kdbulan<'$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by kdgiat,kdoutput, kdakun) as c on  a.kdgiat=c.kdgiat and a.kdoutput=c.kdoutput and a.kdakun=c.kdakun 
left join (select kdgiat, kdoutput, kdakun,  kdkotama, kdsatker, thang, sum(realisasi) as  blnini from realisasi_blu where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by kdgiat,kdoutput, kdakun) as d on  a.kdgiat=d.kdgiat and a.kdoutput=d.kdoutput and a.kdakun=d.kdakun 
where  a.kdkotama='$_SESSION[kdkotama]' and a.kdsatker='$_SESSION[kdsatker]' and a.thang='$_GET[thang]' 
...
FROM blu  a
left join (select  id_pagu, id_realisasi, sum(realisasi) as blnlalu from realisasi_blu where kdbulan<'$_GET[kdbulan]'  and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by id_pagu) as c on a.id_pagu=c.id_pagu
left join (select id_pagu, sum(realisasi) as blnini  from realisasi_blu where kdbulan='$_GET[kdbulan]'  and kdkotama='$_SESSION[kdkotama]' and kdsatker='$_SESSION[kdsatker]' and thang='$_GET[thang]' group by id_pagu) as d on a.id_pagu=d.id_pagu
...
print "<form name='form1' method='GET'  action='realisasiblu/kirimparameter.php' >";
	...
	print "<form name='form1' method='GET'  action='cetak/cetak_mon_blu_satker.php' target='_Blank'>"; 
...
while($k = mysql_fetch_array($ok)){
	...
	if ($pj=='3') {	
	 ...
	 print "<td  valign='top' align='right'></td>";
		...
		print "<a href='media.php?module=inputrealisasiblu&id_pagu=$k[id_pagu]&kdbulan=$_GET[kdbulan]&thang=$_GET[thang]' data-tooltip='Realisasi' data-position='top' class='top'>

...
<?
		 $edit = mysql_query("select * from tunkin where id_tunkin='$_GET[id_tunkin]' ");
		 $row    = mysql_fetch_array($edit);
		 
		  if ($row[jumlah]!='0')    { $jumlah=number_format($row[jumlah],0,',','.'); }  else {  $jumlah=''; }
		  if ($row[pajak]!='0')    { $pajak=number_format($row[pajak],0,',','.'); }  else {  $pajak=''; }

		 $qry = mysql_query("select * from t_bulan where kdbulan='$row[kdbulan]' ");
		 $x    = mysql_fetch_array($qry);
		 
		 $qry1 = mysql_query("select nmkotama from t_kotam where kdkotama='$row[kdkotama]' ");
		 $x1    = mysql_fetch_array($qry1);
		 
		 $qry2 = mysql_query("select nmsatkr from t_satkr where kdkotama='$row[kdkotama]' and kdsatkr='$row[kdsatker]'");
		 $x2    = mysql_fetch_array($qry2);
?>		 

<br>
<center><span class="judul">EDIT REKAP TUNKIN</span></center><br>
<center><div id="bdr">
<div class="form-style-2">
<form action="tunkin/proses.php?aksi=ubah" method="POST"  name="form1">  

    <table width="650" align="center" cellpadding="5">
           <input name="id_tunkin" type="hidden" class="input-field"value="<? echo "$row[id_tunkin]"; ?>" readonly />
			<tr>
				<td width="150" align="right" class="subyek1">BULAN :</td>
			    <td valign="top" ><input name="kdbulan" type="hidden"   class="input-field" value="<? echo "$row[kdbulan]"; ?>"/>
			...
			<tr>
				...
				<td valign="top" ><input name="thang" type="text" size="6" class="input-field"value="<? echo "$row[thang]"; ?>" readonly /></td>
		    </tr>
			
			<tr>
				<td width="150" align="right" class="subyek1">KOTAMA :</td>
				<td valign="top" ><input name="kdkotama" type="hidden"  size="40" class="input-field" value="<? echo "$row[kdkotama]"; ?>"  />
			...
			<tr>
				...
				<td valign="top" ><input name="kdsatker" type="hidden"  size="40" class="input-field" value="<? echo "$row[kdsatker]"; ?>"/>
			...
			<tr>
				...
				<td valign="top" ><?
						...
						<option value='' selected>- - - Pilih - - -</option>";
						 ...
						 if ($data[grade]==$row[grade])
							echo "<option value=$data[grade] selected>$data[grade] | $indeks</option>";
						 else
			...
			<tr>
				...
				<td valign="top" ><? print "<input name='jumlah' type='hidden'  value='$row[jumlah]' class='rounded' style='text-align: right;' />
					  ...
					  style='text-align: right;' value='$jumlah' class='input-field' required='required'>"; ?></td>
			</tr>
			
			<tr>
				<td width="150" align="right" class="subyek1">PAJAK :</td>
				<td valign="top" ><? print "<input name='pajak' type='hidden'  value='$row[pajak]' class='rounded' style='text-align: right;' />
					  ...
					  style='text-align: right;' value='$pajak' class='input-field' required='required'>"; ?></td>
			</tr>

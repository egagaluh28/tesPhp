...
<?php
	     ...
	     $qryy = mysql_query("select id_pagu, kdwasgiat, kdsa, kdjd, kdjenbel, kdprogram, kdgiat, kdoutput, kdakun, kdsakun,  sum(pagurevisi) as pagurevisi, nmitem from bpjs where id_pagu='$_GET[id_pagu]'");
...
<form name="form1"  method="POST"  action="realisasibpjs/proses.php?aksi=simpan">
...
<table align="center" >
	<tr>
		<td><div class='codehim-tombol-biru'><input  type="submit" value="Simpan" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
			...
			<? print "<a href='media.php?module=realisasibpjs&kdbulan=$_GET[kdbulan]&thang=$_GET[thang]' style='text-decoration:none'><input type='button' value='Keluar' ></a>"; ?></div>
...
//	print    "<th   align='center' >NILAI SP2D</th>";
	...
	$sql="SELECT a.*, b.nmbulan from realisasi_bpjs a left join t_bulan b on a.kdbulan=b.kdbulan  where a.id_pagu='$_GET[id_pagu]' order by a.kdbulan, a.tglsp2d";
    $qry=mysql_query($sql);
	...
	print"<tr><td  align='center' valign='top'>$no</td>
			  ...
			  <td  align='center' valign='top'><a href='media.php?module=editrealisasibpjs&id_realisasi=$row[id_realisasi]&id_pagu=$row[id_pagu]&kdbulan=$_GET[kdbulan]&thang=$_GET[thang]' data-tooltip='Edit Realisasi' data-position='top' class='top'><img src='images/edit.png' width='20' ></a></td>
			  <td  align='center' valign='top'><a href=\"realisasibpjs/proses.php?aksi=hapus&id_realisasi=$row[id_realisasi]&id_pagu=$row[id_pagu]&kdbulan=$_GET[kdbulan]&thang=$_GET[thang]\" onClick=\"return confirm('APAKAH ANDA AKAN MENGHAPUS  ~ $row[uraian] ~? ')\" data-tooltip='Hapus Realisasi' data-position='top' class='top'><img src='images/delete.png' width='20' ></td>			  
		  </tr>";	
		  $no++;
     }
	 
	 $jml = mysql_query("SELECT sum(realisasi) as jumlah from realisasi_bpjs	 where  id_pagu ='$_GET[id_pagu]' ");						   

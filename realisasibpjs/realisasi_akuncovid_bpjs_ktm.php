...
left join (select  id_pagu, kdakun,  kdkotama, kdsatker, thang, sum(realisasi) as blnlalu from realisasi_bpjs where kdbulan<'$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]'  and thang='$_GET[thang]' and kdakun='521131' group by kdsatker) as c on   a.kdsatker=c.kdsatker 
left join (select  id_pagu, kdakun,  kdkotama, kdsatker,  thang, sum(realisasi) as  blnini from realisasi_bpjs where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]'  and thang='$_GET[thang]' and kdakun='521131' group by kdsatker) as d on   a.kdsatker=d.kdsatker 
...
sum(a.pagu) as pagu, sum(a.revisi) as revisi, sum(a.pagurevisi) as pagurevisi,    c.blnlalu, d.blnini FROM bpjs a 
left join (select  id_pagu, kdakun,  kdkotama,  thang, sum(realisasi) as blnlalu from realisasi_bpjs where kdbulan<'$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]'  and thang='$_GET[thang]' and kdakun='521131' group by id_pagu) as c on   a.id_pagu=c.id_pagu 
left join (select  id_pagu, kdakun,  kdkotama,  thang, sum(realisasi) as  blnini from realisasi_bpjs where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]'  and thang='$_GET[thang]' and kdakun='521131' group by id_pagu) as d on   a.id_pagu=d.id_pagu 
...
<table  width='98%' align='center'>
    <tr>
		 ...
		 <form name='form1' method='GET'  action='realisasibpjs/kirimparameter_akuncovid_bpjs_ktm.php' >
	...
	print "&nbsp;&nbsp;<a href='cetak/cetak_akuncovid_bpjs_ktm.php?kdbulan=$_GET[kdbulan]&thang=$_GET[thang]&kdkotama=$_SESSION[kdkotama]&kdsatker=$_SESSION[kdsatker]' target='_blank' style='text-decoration:none'><input  type='button' value='Cetak' class='button2'/></a>";
	?> 
...
left join (select  kdkotama,  thang, sum(realisasi) as blnlalu from realisasi_bpjs where kdbulan<'$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]'  and thang='$_GET[thang]' and kdakun='521131' group by kdkotama) as c on   a.kdkotama=c.kdkotama 
left join (select kdkotama,  thang, sum(realisasi) as  blnini from realisasi_bpjs where kdbulan='$_GET[kdbulan]' and kdkotama='$_SESSION[kdkotama]'  and thang='$_GET[thang]' and kdakun='521131' group by kdkotama) as d on   a.kdkotama=d.kdkotama 

...
<br><table  width='1100' align='center' ><tr><td class='judulcontent' align='center'>PEREKAMAN DATA REALISASI</td></tr></table><br>	
 
<?php
		
	     $qryy = mysql_query("select id_pagu, kdwasgiat, kdsa, kdjd, kdjenbel, kdprogram, kdgiat, kdoutput, kdakun, kdsakun,  sum(pagurevisi) as pagurevisi, nmitem from sbsn where id_pagu='$_GET[id_pagu]'");
...
<form name="form1"  method="POST"  action="realisasisbsn/proses.php?aksi=simpan">
...
<table align="center" >
	<tr>
		<td><div class='codehim-tombol-biru'><input  type="submit" value="Simpan" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
			...
			<? print "<a href='media.php?module=realisasisbsn&kdbulan=$_GET[kdbulan]&thang=$_GET[thang]' style='text-decoration:none'><input type='button' value='Keluar' ></a>"; ?></div>
...
// header
	print "<table width='97%'  cellspacing='5' cellpadding='5' align='center' class='bordered'>";
	print "<tr  >";
...
//	print    "<th   align='center' >NILAI SP2D</th>";
	...
	$sql="SELECT a.*, b.nmbulan from realisasi_sbsn a left join t_bulan b on a.kdbulan=b.kdbulan  where a.id_pagu='$_GET[id_pagu]' order by a.kdbulan, a.tglsp2d";
    $qry=mysql_query($sql);
	...
	print"<tr><td  align='center' valign='top'>$no</td>
			  ...
			  <td  align='center' valign='top'><a href='media.php?module=editrealisasisbsn&id_realisasi=$row[id_realisasi]&id_pagu=$row[id_pagu]&kdbulan=$_GET[kdbulan]&thang=$_GET[thang]' data-tooltip='Edit Realisasi' data-position='top' class='top'><img src='images/edit.png' width='20' ></a></td>
			  <td  align='center' valign='top'><a href=\"realisasisbsn/proses.php?aksi=hapus&id_realisasi=$row[id_realisasi]&id_pagu=$row[id_pagu]&kdbulan=$_GET[kdbulan]&thang=$_GET[thang]\" onClick=\"return confirm('APAKAH ANDA AKAN MENGHAPUS  ~ $row[uraian] ~? ')\" data-tooltip='Hapus Realisasi' data-position='top' class='top'><img src='images/delete.png' width='20' ></td>			  
		  </tr>";	
		  $no++;
     }
	 
	 $jml = mysql_query("SELECT sum(nilai_spm) as jspp, sum(nilai_spp) as jspm, sum(realisasi) as jumlah from realisasi_sbsn	 where  id_pagu ='$_GET[id_pagu]' ");						   
     $data1    = mysql_fetch_array($jml);
	 
	 $jumlah = number_format($data1[jumlah],0,',','.');
	 $jspp = number_format($data1[jspp],0,',','.');
	  $jspm = number_format($data1[jspm],0,',','.');
	  
		print"<tr><td  align='center'></td>
			  <td   colspan='4'>&nbsp;&nbsp;&nbsp;&nbsp;<span>J U M L A H</span></td>
			  <td   align='right'><span>$jspp</span></td>
			  <td   colspan='2'></td>	
			  <td   align='right'><span>$jspm</span></td>	
			  <td   colspan='3'></td>	
